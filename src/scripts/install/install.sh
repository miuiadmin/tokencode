...
BIN_PATH="$BIN_DIR/tokencode"
CODEX_HOME_DIR="${TOKENCODE_HOME:-${CODEX_HOME:-$HOME/.tokencode}}"
...
validate_version() {
  ...
  if ! printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-(alpha|beta)(\.[0-9]+)?)?$'; then
    echo "Invalid TokenCode release version: $version. Expected latest or x.y.z[-alpha[.N]|-beta[.N]]." >&2
    exit 1
  fi
}
...
download_file() {
  ...
  echo "curl or wget is required to install TokenCode." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/miuiadmin/tokencode/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}

release_metadata_url() {
  resolved_version="$1"

  printf 'https://api.github.com/repos/miuiadmin/tokencode/releases/tags/rust-v%s\n' "$resolved_version"
}
...
resolve_release() {
  ...
  if [ "$normalized_version" = "latest" ]; then
    ...
    metadata_url="https://api.github.com/repos/miuiadmin/tokencode/releases/latest"
  else
  ...
  if ! release_json="$(download_text "$metadata_url")"; then
    echo "Could not fetch GitHub release metadata for TokenCode $requested_release. GitHub API may be unavailable or rate limited." >&2
    exit 1
  fi
  ...
  if [ "$normalized_version" = "latest" ]; then
    ...
    if [ -z "$resolved_version" ]; then
      echo "Failed to resolve the latest TokenCode release version." >&2
      exit 1
    fi
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the TokenCode download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded TokenCode archive checksum did not match expected digest." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install TokenCode." >&2
    exit 1
  fi
}
...
add_to_path() {
  ...
  begin_marker="# >>> TokenCode installer >>>"
  end_marker="# <<< TokenCode installer <<<"
...
current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/bin/tokencode" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi

  version="$(version_from_binary "$CURRENT_LINK/tokencode" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_codex() {
  command -v tokencode 2>/dev/null || true
}
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && tokencode"
      step "Future terminals: open a new terminal and run: tokencode"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: tokencode"
      step "Future terminals: open a new terminal and run: tokencode"
      ;;
  esac
}
...
maybe_launch_codex_now() {
  if prompt_yes_no "Start TokenCode now?"; then
    step "Launching TokenCode"
    "$BIN_PATH"
  fi
}
...
detect_conflicting_install() {
  ...
  step "Detected existing $manager-managed TokenCode at $existing_path"
  warn "Multiple managed TokenCode installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  if prompt_yes_no "Uninstall the existing $conflict_manager-managed TokenCode now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed TokenCode. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed TokenCode installed. PATH order will determine which tokencode runs."
  fi
}
...
install_package_release() {
  ...
  chmod 0755 "$stage_release/bin/tokencode" "$stage_release/codex-path/rg"
  if [ -f "$stage_release/codex-resources/bwrap" ]; then
    chmod 0755 "$stage_release/codex-resources/bwrap"
  fi
  ln -sf "bin/tokencode" "$stage_release/tokencode"
...
install_legacy_platform_npm_release() {
  ...
  cp "$vendor_root/codex/codex" "$stage_release/tokencode"
  cp "$vendor_root/path/rg" "$stage_release/codex-resources/rg"
  chmod 0755 "$stage_release/tokencode" "$stage_release/codex-resources/rg"
  ...
  case "$layout" in
    package)
      [ -f "$release_dir/codex-package.json" ] &&
        [ -x "$release_dir/bin/tokencode" ] &&
        [ -x "$release_dir/tokencode" ] &&
...
release_codex_relative_path() {
  ...
  if [ -x "$release_dir/bin/tokencode" ]; then
    printf 'bin/tokencode\n'
  else
    printf 'tokencode\n'
  fi
}
...
if release_asset_exists "$package_asset" &&
  ...
  echo "Could not find TokenCode package or platform npm release assets for TokenCode $resolved_version." >&2
  exit 1
fi
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating TokenCode CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating TokenCode CLI"
else
  step "Installing TokenCode CLI"
fi
...
if ! release_dir_is_complete "$release_dir" "$resolved_version" "$vendor_target" "$install_layout"; then
  ...
  step "Downloading TokenCode CLI"
...
printf 'TokenCode CLI %s installed successfully.\n' "$resolved_version"
maybe_launch_codex_now
