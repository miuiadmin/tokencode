...
function Assert-ValidReleaseVersion {
    ...
    if ($Version -cne "latest" -and $Version -cnotmatch "^[0-9]+\.[0-9]+\.[0-9]+(?:-(?:alpha|beta)(?:\.[0-9]+)?)?$") {
        throw "Invalid TokenCode release version: $Version. Expected latest or x.y.z[-alpha[.N]|-beta[.N]]."
    }
}
...
function Test-ArchiveDigest {
    ...
    if ($actualDigest -ne $ExpectedDigest) {
        throw "Downloaded TokenCode archive checksum did not match expected digest. Expected $ExpectedDigest but got $actualDigest."
    }
}
...
function Resolve-Release {
    ...
    if ($normalizedVersion -eq "latest") {
        ...
        $metadataUri = "https://api.github.com/repos/miuiadmin/tokencode/releases/latest"
    } else {
        $resolvedVersion = $normalizedVersion
        $requestedRelease = $resolvedVersion
        $metadataUri = "https://api.github.com/repos/miuiadmin/tokencode/releases/tags/rust-v$resolvedVersion"
    }

    try {
        $releaseMetadata = Invoke-RestMethod -Uri $metadataUri
    } catch {
        throw "Could not fetch GitHub release metadata for TokenCode $requestedRelease. GitHub API may be unavailable or rate limited. $($_.Exception.Message)"
    }

    if ($normalizedVersion -eq "latest") {
        if (-not $releaseMetadata.tag_name) {
            throw "Failed to resolve the latest TokenCode release version."
        }
...
function Get-CurrentInstalledVersion {
    ...
    $standaloneVersion = Get-VersionFromBinary -CodexPath (Join-Path $StandaloneCurrentDir "bin\tokencode.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }

    $standaloneVersion = Get-VersionFromBinary -CodexPath (Join-Path $StandaloneCurrentDir "tokencode.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }
...
function Test-OldStandaloneBinLayout {
    ...
    $requiredFiles = @("tokencode.exe", "rg.exe")
...
function Move-OldStandaloneBinIfApproved {
    ...
    Write-Step "We found an older TokenCode install at $VisibleBinDir"
    Write-WarningStep "To continue, TokenCode needs to update the install at this path."
    if (-not (Prompt-YesNo "Replace it with the current TokenCode setup now?")) {
...
function Test-ReleaseIsComplete {
    ...
    switch ($Layout) {
        ...
        default {
            throw "Unknown TokenCode installer layout: $Layout"
        }
    }
...
function Get-ExistingCodexCommand {
    $existing = Get-Command tokencode -ErrorAction SilentlyContinue
    if ($null -eq $existing) {
        return $null
    }
...
function Get-ConflictingInstall {
    ...
    Write-Step "Detected existing $manager-managed TokenCode at $existingPath"
    Write-WarningStep "Multiple managed TokenCode installs can be ambiguous because PATH order decides which one runs."
...
function Maybe-HandleConflictingInstall {
    ...
    if (Prompt-YesNo "Uninstall the existing $manager-managed TokenCode now?") {
        Write-Step "Running: $uninstallCommand $($uninstallArgs -join ' ')"
        try {
            & $uninstallCommand @uninstallArgs
        } catch {
            Write-WarningStep "Failed to uninstall the existing $manager-managed TokenCode. Continuing with the standalone install."
        }
    } else {
        Write-WarningStep "Leaving the existing $manager-managed TokenCode installed. PATH order will determine which tokencode runs."
    }
}
...
function Test-VisibleCodexCommand {
    ...
    $codexCommand = Join-Path $VisibleBinDir "tokencode.exe"
    & $codexCommand --version *> $null
    if ($LASTEXITCODE -ne 0) {
        throw "Installed TokenCode command failed verification: $codexCommand --version"
    }
}
...
if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Error "TokenCode requires a 64-bit version of Windows."
    exit 1
}
...
$codexHome = if (-not [string]::IsNullOrWhiteSpace($env:TOKENCODE_HOME)) {
    $env:TOKENCODE_HOME
} elseif (-not [string]::IsNullOrWhiteSpace($env:CODEX_HOME)) {
    $env:CODEX_HOME
} else {
    Join-Path $env:USERPROFILE ".tokencode"
}
...
$defaultVisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\TokenCode\bin"
...
if (-not [string]::IsNullOrWhiteSpace($currentVersion) -and $currentVersion -ne $resolvedVersion) {
    Write-Step "Updating TokenCode CLI from $currentVersion to $resolvedVersion"
} elseif (-not [string]::IsNullOrWhiteSpace($currentVersion)) {
    Write-Step "Updating TokenCode CLI"
} else {
    Write-Step "Installing TokenCode CLI"
}
...
if ($null -eq $packageMetadata -or $null -eq $checksumMetadata) {
    ...
    if ($null -ne $packageMetadata) {
        ...
        throw "Could not find TokenCode package or platform npm release assets for TokenCode $resolvedVersion."
    }
...
try {
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target -Layout $installLayout)) {
            ...
            Write-Step "Downloading TokenCode CLI"
            ...
            if ($installLayout -eq "Package") {
                ...
                if (-not (Test-PackageContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded TokenCode package archive did not contain the expected package layout."
                }
            } else {
                ...
                if (-not (Test-LegacyPlatformNpmContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded TokenCode npm archive did not contain the expected legacy platform package layout."
                }
            }
...
Write-Step "Current PowerShell session: tokencode"
Write-Step "Future PowerShell windows: open a new PowerShell window and run: tokencode"
Write-Host "TokenCode CLI $resolvedVersion installed successfully."

$codexCommand = Join-Path $visibleBinDir "tokencode.exe"
if (Prompt-YesNo "Start TokenCode now?") {
    Write-Step "Launching TokenCode"
    & $codexCommand
}
