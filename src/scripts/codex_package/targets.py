...
PACKAGE_VARIANTS: dict[str, PackageVariant] = {
    # 变体的 key/name 是内部选择器与包元数据字段，保留 codex 命名（用户不可见）。
    # 但 cargo_bin / executable_stem 必须指向真实的二进制名 tokencode
    # （CLI [[bin]] 已改名，旧的 codex 会让 cargo build --bin 找不到目标、
    # 且包内入口 bin/tokencode 需与安装器探测路径一致）。
    "codex": PackageVariant(
        name="codex",
        cargo_bin="tokencode",
        executable_stem="tokencode",
    ),
