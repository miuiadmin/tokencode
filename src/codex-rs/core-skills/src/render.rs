...
pub const SKILL_DESCRIPTION_TRUNCATED_WARNING: &str = "Skill descriptions were shortened to fit the skills context budget. TokenCode can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest.";
pub const SKILL_DESCRIPTION_TRUNCATED_WARNING_WITH_PERCENT: &str = "Skill descriptions were shortened to fit the 2% skills context budget. TokenCode can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest.";
...
mod tests {
    ...
    fn budgeted_rendering_warns_when_average_description_truncation_exceeds_threshold() {
        ...
        assert_eq!(
            ...
            Some(
                "Skill descriptions were shortened to fit the skills context budget. TokenCode can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest."
                    .to_string()
            )
        );
    }
    ...
    fn outcome_rendering_uses_aliases_when_they_allow_more_skills_to_fit() {
        let root = test_path_buf(
            "/Users/xl/.tokencode/plugins/cache/openai-curated/example/hash1234567890/skills-with-a-very-long-shared-prefix",
        )
        .abs();
        let skills = (0..12)
            .map(|index| {
    ...
    fn outcome_rendering_uses_marketplace_root_for_single_skill_plugin_versions() {
        let github_root =
            test_path_buf("/Users/xl/.tokencode/plugins/cache/openai-curated/github/hash123/skills")
                .abs();
        let marketplace_root = test_path_buf("/Users/xl/.tokencode/plugins/cache/openai-curated").abs();
    ...
    fn outcome_rendering_uses_each_skill_root_for_multiple_roots_in_one_plugin_version() {
        ...
        let extra_root = test_path_buf(
            "/Users/xl/.tokencode/plugins/cache/openai-curated/github/hash123/extra-skills",
        )
        .abs();
    ...
    fn outcome_rendering_extracts_plugin_marketplace_root_for_multiple_plugins() {
        let github_root =
            test_path_buf("/Users/xl/.tokencode/plugins/cache/openai-curated/github/hash123/skills")
                .abs();
        let slack_root =
            test_path_buf("/Users/xl/.tokencode/plugins/cache/openai-curated/slack/hash456/skills")
                .abs();
        let marketplace_root = test_path_buf("/Users/xl/.tokencode/plugins/cache/openai-curated").abs();
        let github = skill_with_path("github:gh-fix-ci", &github_root.join("gh-fix-ci/SKILL.md"));
    ...
    fn outcome_rendering_uses_one_marketplace_root_for_multiple_plugin_versions() {
        let skills_root =
            test_path_buf("/Users/xl/.tokencode/plugins/cache/openai-curated/github/hash123/skills")
                .abs();
        let extra_root = test_path_buf(
            "/Users/xl/.tokencode/plugins/cache/openai-curated/github/hash456/extra-skills",
        )
        .abs();
        let marketplace_root = test_path_buf("/Users/xl/.tokencode/plugins/cache/openai-curated").abs();
        let fix_ci = skill_with_path("github:gh-fix-ci", &skills_root.join("gh-fix-ci/SKILL.md"));
