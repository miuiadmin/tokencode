...
/// Minimal launcher that initializes the TokenCode app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "TokenCode", version, about = "Bootstrap TokenCode app-server", long_about = None)]
struct Cli {
    /// Path to the `codex` CLI binary. When set, requests use stdio by
    /// spawning `tokencode app-server` as a child process.
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `tokencode app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/codex-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `tokencode app-server --listen`.
    ...
    /// Send a user message through the TokenCode app-server.
    SendMessage {
        /// User message to send to TokenCode.
        user_message: String,
    },
    ...
    /// Fetch the current account rate limits from the TokenCode app-server.
    GetAccountRateLimits,
    /// List the available models from the TokenCode app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the TokenCode app-server.
...
fn serve(codex_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    ...
    println!("started tokencode app-server");
...
impl CodexClient {
    ...
    fn spawn_stdio_with_env(
        ...
        let stdin = codex_app_server
            .stdin
            .take()
            .context("tokencode app-server stdin unavailable")?;
        let stdout = codex_app_server
            .stdout
            .take()
            .context("tokencode app-server stdout unavailable")?;
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    ...
                    title: Some("TokenCode Toy App Server".to_string()),
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to tokencode app-server")?;
                    return Ok(());
                }
                bail!("tokencode app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from tokencode app-server")?;
                if bytes == 0 {
                    bail!("tokencode app-server closed stdout");
                }
                Ok(response_line)
            }
