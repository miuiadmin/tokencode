...
#[command(
    ...
    about = "Run one TokenCode turn through ThreadManager and print mapped notifications as newline-delimited JSON."
)]
struct Args {
...
async fn run_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let NewThread {
        ...
        .start_thread(config)
        .await
        .context("start TokenCode thread")?;
    ...
    turn_output?;
    shutdown_result.context("shut down TokenCode thread")?;

    Ok(())
}

fn new_config(model: Option<String>, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<Config> {
    let codex_home = find_codex_home().context("find TokenCode home")?;
...
async fn run_turn(thread: &CodexThread, thread_id: &str, prompt: String) -> anyhow::Result<()> {
    ...
    loop {
        let event = thread.next_event().await.context("read TokenCode event")?;
