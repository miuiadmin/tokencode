...
async fn marketplace_policy_projection_disables_installed_plugin_and_invalidates_cache() {
    let codex_home = TempDir::new().expect("create TokenCode home");
    write_plugin(
...
fn init_git_repo(repo: &Path) {
    ...
    run_git(repo, &["config", "user.name", "TokenCode Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(
    ...
    PluginsManager::new_with_options(codex_home.to_path_buf(), Some(Product::TokenCode), auth_mode)
...
"#,
    ...
    write_file(
        &repo_root.join(".tokencode/config.toml"),
        r#"[[skills.config]]
...
async fn load_plugins_ignores_project_config_files() {
    ...
    write_file(
        &project_root.join(".tokencode/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_codex_folder: AbsolutePathBuf::try_from(project_root.join(".tokencode")).unwrap(),
            },
