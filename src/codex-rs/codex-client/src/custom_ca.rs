//! Custom CA handling for TokenCode outbound HTTP and websocket clients.
//!
//! TokenCode constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!   because TokenCode itself commonly runs spawned test processes under seatbelt, so this is not just
...
/// Builds a reqwest client that honors TokenCode custom CA environment variables.
///
...
/// easy mistake to make when adding a new outbound TokenCode HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a TokenCode custom CA bundle is configured.
///
...
fn maybe_build_rustls_client_config_with_env(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the TokenCode custom
...
trait EnvSource {
    ...
    ///
    /// `CODEX_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the TokenCode-specific override.
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for TokenCode CA overrides.
    ///
    /// This accepts a few common real-world variants so TokenCode behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// TokenCode only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
