...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run TokenCode after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
    ...
    /// Use the current Windows user as the TokenCode user.
    #[arg(
        long = "current-user",
    ...
    /// CODEX_HOME for the TokenCode user. Required with --user.
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`tokencode sandbox setup` currently requires --elevated");
        }
    }
}
