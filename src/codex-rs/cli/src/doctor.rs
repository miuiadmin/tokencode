//! Implements the `tokencode doctor` diagnostic report.
//!
...
/// Options for building a local TokenCode diagnostic report.
///
...
///
/// This is the CLI entry point for tokencode doctor. It does not repair issues;
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun tokencode doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load TokenCode config")
}
...
/// JSON support report emitted by `tokencode doctor --json`.
///
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    if path_entries.len() > 1 {
        details.push(format!("PATH tokencode entries: {}", path_entries.len()));
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary =
                    "npm install -g tokencode would update a different install".to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update TokenCode so the JS shim provides CODEX_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("tokencode");
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run tokencode login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no TokenCode credentials were found",
        )
        .details(details)
        .remediation("Run tokencode login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run tokencode login again."),
    }
}
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/tokencode");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("tokencode")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/tokencode");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("tokencode"),
            }
        );
    }
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running TokenCode."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running TokenCode.".to_string())
        );
    }
