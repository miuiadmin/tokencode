...
#[clap(
    ...
    // `tokencode-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `tokencode` command name that users run.
    bin_name = "tokencode",
    override_usage = "tokencode [OPTIONS] [PROMPT]\n       tokencode [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    /// Run TokenCode non-interactively.
    ...
    /// Manage external MCP servers for TokenCode.
    Mcp(McpCli),

    /// Manage TokenCode plugins.
    Plugin(PluginCli),

    /// Start TokenCode as an MCP server (stdio).
    ...
    /// Launch the TokenCode desktop app (opens the app installer if missing).
    #[cfg(any(target_os = "macos", target_os = "windows"))]
    ...
    /// Update TokenCode to the latest version.
    Update,

    /// Diagnose local TokenCode installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a TokenCode-provided sandbox.
    ...
    /// Apply the latest diff produced by TokenCode agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from TokenCode Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of TokenCode.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | tokencode login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv CODEX_ACCESS_TOKEN | tokencode login --with-access-token`)"
    )]
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for TokenCode tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating TokenCode via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart TokenCode.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`tokencode update` is not available in debug builds. Install a release build of TokenCode to use this command."
        );
    }
    ...
    #[cfg(not(debug_assertions))]
    {
        let Some(action) = codex_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the TokenCode installation method. Please update manually."
            );
        };
        run_update_action(action)
    }
    ...
    match subcommand {
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | tokencode login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`tokencode sandbox` is not supported on this operating system");
            }
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `tokencode mcp`: `codex`, `tokencode exec`, `tokencode review`, `tokencode resume`, `tokencode archive`, `tokencode delete`, `tokencode unarchive`, `tokencode fork`, `tokencode mcp`, `tokencode sandbox`, and `tokencode debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let codex_self_exe = arg0_paths
        .codex_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("TokenCode executable path is not configured"))?;
...
async fn load_exec_server_remote_auth_provider(
    ...
    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `tokencode login` or set CODEX_API_KEY",
    )
    .await?;
...
async fn run_interactive_tui(
    ...
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". TokenCode's interactive TUI may not work in this terminal."
        );
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged TokenCode local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
/// Build the final `TuiCli` for a `tokencode resume` invocation.
...
/// Build the final `TuiCli` for a `tokencode fork` invocation.
...
mod tests {
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let codex_home = find_codex_home().expect("tokencode home");
        codex_app_server::app_server_control_socket_path(&codex_home)
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        ...
        assert!(
            help.contains("Usage: tokencode plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: tokencode plugin marketplace add"),
            ("list", "Usage: tokencode plugin marketplace list"),
            ("upgrade", "Usage: tokencode plugin marketplace upgrade"),
            ("remove", "Usage: tokencode plugin marketplace remove"),
        ] {
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run tokencode resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run tokencode resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `tokencode mcp`"
        );
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `tokencode remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `tokencode app-server proxy`"
        );
    }
