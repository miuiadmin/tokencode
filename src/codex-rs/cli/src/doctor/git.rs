...
fn git_check_from_inputs(inputs: GitCheckInputs) -> DoctorCheck {
    ...
    if inputs.selected_git.is_some() && inputs.git_version.is_none() {
        ...
        check = check.issue(
            ...
            .remedy("Fix the selected Git executable or PATH so TokenCode can inspect Git metadata.")
            .field("git version")
            .field("selected git"),
        );
        ...
        check = check.issue(
            ...
            .remedy("Install Git or fix PATH so TokenCode can inspect repository metadata.")
            .field("selected git"),
        );
        ...
        old_windows_git_warning(inputs.git_version.as_deref(), cfg!(windows))
    {
        ...
        check = check.issue(
            DoctorIssue::new(CheckStatus::Warning, cause)
                ...
                .remedy(
                    "Update Git for Windows or the bundled Git executable TokenCode resolves first.",
                )
                .field("git version")
                .field("selected git"),
        );
