    ...
    if !scan.files.is_empty() {
        check = check
            .issue(
                ...
                .remedy("Start TokenCode with no state DB present so startup backfill can create it from rollout files."),
        )
            .remediation(
                "Start TokenCode with no state DB present so startup backfill can create it from rollout files.",
            );
    }
...
mod tests {
    ...
    async fn thread_inventory_check_warns_for_missing_stale_and_mismatched_rows() {
        ...
        assert!(check.issues.iter().all(|issue| {
            !issue
                .remedy
                .as_deref()
                .is_some_and(|remedy| remedy.starts_with("Restart TokenCode"))
        }));
        assert!(
            check
                .details
    ...
    impl Fixture {
        async fn new() -> Self {
            let codex_home = TempDir::new().expect("tokencode home");
