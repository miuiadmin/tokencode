...
//! interactive-session layers. Direct `tokencode login` intentionally does less: it preserves the
...
/// Installs a small file-backed tracing layer for direct `tokencode login` flows.
///
...
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    ...
    // Direct `tokencode login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `tokencode login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | tokencode login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv CODEX_ACCESS_TOKEN | tokencode login --with-access-token`.",
...
/// `tokencode login` working in environments where device-code may be disabled/feature-gated.
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let codex_home = tempdir().expect("create temporary TokenCode home");
        login_with_api_key(
            codex_home.path(),
            "sk-existing",
