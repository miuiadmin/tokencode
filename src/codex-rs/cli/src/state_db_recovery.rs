...
pub(crate) fn print_auto_backup_start(startup_error: &LocalStateDbStartupError) {
    eprintln!("TokenCode couldn't start because its local database appears to be damaged.");
    eprintln!("Moving the damaged local database aside so TokenCode can rebuild it from saved data.");
...
pub(crate) fn confirm_fresh_start_rebuild(
    ...
    eprintln!("TokenCode rebuilt its local database.");
    eprintln!(
        "TokenCode detected a damaged local database, moved it into a backup folder, and will continue startup with a fresh database."
    );
...
pub(crate) fn print_diagnostic_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("TokenCode couldn't start because its local database appears to be damaged.");
    eprintln!("Run `tokencode doctor` to check your setup and get next-step guidance.");
...
pub(crate) fn print_locked_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("TokenCode couldn't start because another TokenCode process is using its local data.");
    eprintln!("Quit any other copies of TokenCode that may still be running, then try again.");
    print_technical_details(startup_error);
}
