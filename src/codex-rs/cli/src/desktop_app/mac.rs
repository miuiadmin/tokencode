...
const CODEX_DMG_URL_ARM64: &str = "https://persistent.oaistatic.com/codex-app-prod/TokenCode.dmg";
const CODEX_DMG_URL_X64: &str =
    "https://persistent.oaistatic.com/codex-app-prod/TokenCode-latest-x64.dmg";
...
pub async fn run_mac_app_open_or_install(
    ...
    if let Some(app_path) = find_existing_codex_app_path() {
        eprintln!(
            "Opening TokenCode Desktop at {app_path}...",
            app_path = app_path.display()
        );
        open_codex_app(&app_path, &workspace).await?;
        return Ok(());
    }
    eprintln!("TokenCode Desktop not found; downloading installer...");
    ...
    let installed_app = download_and_install_codex_to_user_applications(&download_url)
        .await
        .context("failed to download/install TokenCode Desktop")?;
    eprintln!(
        "Launching TokenCode Desktop from {installed_app}...",
...
fn candidate_codex_app_paths() -> Vec<PathBuf> {
    let mut paths = vec![PathBuf::from("/Applications/TokenCode.app")];
    if let Some(home) = std::env::var_os("HOME") {
        paths.push(PathBuf::from(home).join("Applications").join("TokenCode.app"));
    }
    paths
}
...
async fn download_and_install_codex_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    ...
    let dmg_path = tmp_root.join("TokenCode.dmg");
    download_dmg(dmg_url, &dmg_path).await?;

    eprintln!("Mounting TokenCode Desktop installer...");
    ...
    let result = async {
        let app_in_volume = find_codex_app_in_mount(&mount_point)
            .context("failed to locate TokenCode.app in mounted dmg")?;
...
async fn install_codex_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        eprintln!(
            "Installing TokenCode Desktop into {applications_dir}...",
        ...
        let dest_app = applications_dir.join("TokenCode.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install TokenCode.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install TokenCode.app to any applications directory");
}
...
fn find_codex_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("TokenCode.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
mod tests {
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tCodex\t/Volumes/TokenCode\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/TokenCode")
        );
    }

    #[test]
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tCodex Installer\t/Volumes/TokenCode Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/TokenCode Installer")
        );
    }
