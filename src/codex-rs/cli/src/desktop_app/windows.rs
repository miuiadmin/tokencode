...
pub async fn run_windows_app_open_or_install(
    ...
    if codex_app_is_installed().await? {
        eprintln!("Opening TokenCode Desktop workspace {display_workspace}...");
        open_url(&codex_new_thread_url(&workspace_path)).await?;
        return Ok(());
    }

    eprintln!("TokenCode Desktop not found; opening Windows installer...");
    ...
    eprintln!("After installing TokenCode Desktop, open workspace {display_workspace}.");
    Ok(())
}
...
async fn codex_app_is_installed() -> anyhow::Result<bool> {
    let output = Command::new("powershell.exe")
        .arg("-NoProfile")
        .arg("-Command")
        .arg("Get-StartApps -Name 'TokenCode' | Select-Object -First 1 -ExpandProperty AppID")
        .output()
        .await
