...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that TokenCode should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: TokenCode can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so TokenCode will call the MCP echo tool
    ...
    // Phase 3: configure TokenCode with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert TokenCode begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.codex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so TokenCode will call the OAuth-backed
    ...
    // Phase 4: configure TokenCode with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert TokenCode begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.codex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the TokenCode fixture.
