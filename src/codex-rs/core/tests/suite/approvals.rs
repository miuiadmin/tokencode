    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured TokenCode helper binary
...
/// same sandbox with `tokencode sandbox -P`. The nested invocation must also pass
...
/// profile names before passing one to `tokencode sandbox -P`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Print an ssh command that recreates the current TokenCode sandbox remotely."
    )
...
"#
    ...
    assert_eq!(
        sandbox_argv.len(),
        9,
        "expected tokencode sandbox ... bash -lc CMD"
    );
    assert_eq!(
        sandbox_argv[..8],
        [
        ...
        "remote_bash.py should use the allowlisted inherited profile and managed configuration to reconstruct the TokenCode sandbox"
    );
