...
// Use this for new `tokencode exec` subprocess tests in this file. These commands
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // TokenCode to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `tokencode exec --profile ...` preserves the active user config
