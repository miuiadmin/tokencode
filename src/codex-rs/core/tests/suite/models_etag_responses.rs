...
async fn refresh_models_on_models_etag_mismatch_and_avoid_duplicate_models_fetch() -> Result<()> {
    ...
    // 1) On spawn, TokenCode fetches /models and stores the ETag.
    ...
    // 2) If the server sends a different X-Models-Etag on /responses, TokenCode refreshes /models.
    ...
    // Second /responses request (tool output) includes the same X-Models-Etag; TokenCode should not
    ...
    let refresh_req = refresh_models_mock
        ...
        .next()
        .expect("one request");
    // Ensure TokenCode includes client_version on refresh. (This is a stable signal that we're using the /models client.)
    assert!(
        refresh_req
            .url
            .query_pairs()
