...
//!
//! `ModelClient` is intended to live for the lifetime of a TokenCode session and holds the stable
...
///
/// This holds configuration and state that should be shared across turns within a TokenCode session
...
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each TokenCode turn. Reusing it across turns would replay
/// the previous turn's sticky-routing token into the next turn, which violates the client/server
/// contract and can cause routing bugs.
pub struct ModelClientSession {
    client: ModelClient,
    /// 该 turn 实际使用的 provider（turn 级，跟随 model 切换）。
    ///
    /// provider 选择跟随 model：切模型时由 [`ModelClientSession::with_provider`] 注入按
    /// `ModelInfo.provider_id` 解析出的 provider。stream 路径读 turn 级 `self.provider`（而非
    /// 会话默认的 `state.provider`），使 wire 协议与 base_url 随模型切换。默认
    /// （prewarm / compact 等无 turn_context 的路径）复用会话默认 provider，行为与现状一致。
    provider: SharedModelProvider,
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a TokenCode session. Per-turn values
    ...
    /// WebSocket 是否启用，按指定 provider 的能力判断（支持 turn 级 provider 解析）。
    fn responses_websocket_enabled_with(&self, provider_info: &ModelProviderInfo) -> bool {
    ...
    pub fn responses_websocket_enabled(&self) -> bool {
        self.responses_websocket_enabled_with(self.state.provider.info())
    }
    ...
    async fn current_client_setup(&self) -> Result<CurrentClientSetup> {
        self.current_client_setup_with(&self.state.provider).await
    }
...
impl ModelClientSession {
    /// 用按模型解析出的 provider 覆盖会话默认 provider（turn 级注入）。
    ///
    /// 切模型时调用方依据 `ModelInfo.provider_id` 解析出 provider 传入，stream 路径
    /// 读 `self.provider` 使 wire 协议与 base_url 随模型切换。
    pub fn with_provider(mut self, provider: SharedModelProvider) -> Self {
        self.provider = provider;
        self
    }

    /// 该 turn 的 provider 是否启用 WebSocket（按 turn 级 provider 能力判断）。
    fn responses_websocket_enabled(&self) -> bool {
        self.client
            .responses_websocket_enabled_with(self.provider.info())
    }

    /// 解析该 turn 的 auth + provider 配置（用 turn 级 provider）。
    async fn current_client_setup(&self) -> Result<CurrentClientSetup> {
        self.client.current_client_setup_with(&self.provider).await
    }
    ...
    async fn stream_responses_api(
        ...
        // HTTP 流式 turn：请求 / 选项构建 + 401 重试 + provider 错误映射统一在
        // `stream_via_language_model` 内；Responses 与 Chat 共用同一套逻辑，仅 adapter
        // 工厂与 endpoint 不同。WebSocket 前奏由调用方（dispatch）负责。
        self.stream_via_language_model(
            ...
            |transport, provider, auth, request_telemetry, sse_telemetry| {
                OpenaiResponsesAdapter::new(transport, provider, auth)
    ...
    /// 经 OpenAI Chat Completions adapter 流式跑一个 turn（HTTP，无 WebSocket 前奏）。
    ///
    /// 与 `stream_responses_api` 共用 `stream_via_language_model`：请求 / 选项构建、401 重试、
    /// provider 错误映射完全一致；差异仅在 adapter（`OpenaiChatAdapter`）与 endpoint 标签。
    /// Chat 协议不支持 Responses 的 WebSocket 通道，故无 WS 前奏，直接走 HTTP。
    #[allow(clippy::too_many_arguments)]
    async fn stream_chat_api(
        ...
        self.stream_via_language_model(
            ...
            CHAT_ENDPOINT,
            |transport, provider, auth, request_telemetry, sse_telemetry| {
                OpenaiChatAdapter::new(transport, provider, auth)
                    .with_telemetry(request_telemetry, sse_telemetry)
            },
        )
        .await
    ...
    /// 经 Anthropic Messages adapter 流式跑一个 turn（HTTP，无 WebSocket 前奏）。
    ///
    /// 与 `stream_responses_api` / `stream_chat_api` 共用 `stream_via_language_model`：请求 / 选项
    /// 构建、401 重试、provider 错误映射完全一致；差异仅在 adapter（`AnthropicAdapter`，其构造时
    /// 由 `AnthropicAuth` 把内层 Bearer 改写为 `x-api-key` + `anthropic-version`）与 endpoint 标签。
    /// Anthropic Messages 协议无 WebSocket 通道，故无 WS 前奏，直接走 HTTP。
    #[allow(clippy::too_many_arguments)]
    async fn stream_anthropic_api(
        ...
        self.stream_via_language_model(
            ...
            ANTHROPIC_ENDPOINT,
            |transport, provider, auth, request_telemetry, sse_telemetry| {
                AnthropicAdapter::new(transport, provider, auth)
                    .with_telemetry(request_telemetry, sse_telemetry)
            },
        )
        .await
    ...
    /// → 构建 transport / telemetry / 请求 / 选项 → 用 `build_adapter` 装配具体 adapter →
    /// `adapter.stream(...)` → 成功则 `map_response_stream` 归一，401 则刷新鉴权重试，其余
    /// 错误经 `map_api_error` 映射后终止。`build_adapter` 决定具体协议（Responses / Chat / …），
    /// 其余字节级一致——新增协议 adapter 只需提供工厂，无需复制本循环。
    #[allow(clippy::too_many_arguments)]
    async fn stream_via_language_model<A, F>(
        &self,
        prompt: &Prompt,
        model_info: &ModelInfo,
        session_telemetry: &SessionTelemetry,
    ...
    where
        ...
        F: Fn(
            ...
            ApiProvider,
            SharedAuthProvider,
            Option<Arc<dyn RequestTelemetry>>,
    ...
    pub async fn stream(
        ...
        let adapter_type = self.provider.info().adapter_type();
        match adapter_type {
            AdapterType::OpenaiResponses => {
                if self.responses_websocket_enabled() {
            ...
            // OpenAI Chat Completions adapter。HTTP-only——Chat 协议无 WebSocket
            // 通道，不经 WS 前奏，直接走 stream_chat_api（与 stream_responses_api 共用同一套
            // 401 重试 / provider 错误映射）。
            AdapterType::OpenaiChat => {
            ...
            // Anthropic Messages adapter。HTTP-only——Anthropic 协议无 WebSocket 通道，
            // 不经 WS 前奏，直接走 stream_anthropic_api（认证由 adapter 内层 AnthropicAuth
            // 把 Bearer 改写为 x-api-key + anthropic-version）。
    ...
    /// Permanently disables WebSockets for this TokenCode session and resets WebSocket state.
    ///
...
///
/// These headers implement TokenCode-specific conventions:
///
...
/// 把中立层错误还原为 `ApiError`：Passthrough 内的具体协议错误 downcast 取回，
/// 保证 core 的 401 重试 / provider 错误映射等分类处理路径字面零改动；非 ApiError 的
/// 透传错误（理论上不会出现——adapter 总是透传 ApiError）退化为 Stream 错误。
fn unified_error_to_api_error(err: UnifiedError) -> ApiError {
    match err {
        UnifiedError::Passthrough(boxed) => match boxed.downcast::<ApiError>() {
            Ok(api_error) => *api_error,
            Err(other) => ApiError::Stream(other.to_string()),
        },
        UnifiedError::Mapping(msg) => ApiError::Stream(format!("unified mapping error: {msg}")),
    }
}

fn map_response_stream(
    api_stream: UnifiedEventStream,
    session_telemetry: SessionTelemetry,
...
where
    S: futures::Stream<Item = std::result::Result<UnifiedEvent, UnifiedError>>
        + Unpin
        + Send
        + 'static,
{
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            // 边界转换：中立事件 / 错误 → core 既有的 ResponseEvent / ApiError，
            // 使下方 16 变体 match 与错误处理分支字面零改动。
            let event = event
