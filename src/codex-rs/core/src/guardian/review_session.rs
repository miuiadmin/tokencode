...
impl GuardianReviewSessionManager {
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, codex: TokenCode) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
    ...
    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, codex: TokenCode) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            codex.session.get_config().await.as_ref(),
            codex.session.user_instructions().await,
        );
...
async fn interrupt_and_drain_turn(codex: &TokenCode, expected_turn_id: &str) -> anyhow::Result<()> {
