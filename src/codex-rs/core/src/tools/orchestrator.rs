...
impl ToolOrchestrator {
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        match first_result {
            ...
            Err(ToolError::TokenCode(CodexErr::Sandbox(SandboxErr::Denied {
                output,
                ...
                if network_policy_decision.is_some() && network_approval_context.is_none() {
                    ...
                    return Err(ToolError::TokenCode(CodexErr::Sandbox(SandboxErr::Denied {
                        output,
                        network_policy_decision,
                    })));
                }
...
fn sandbox_outcome_from_tool_error(err: &ToolError) -> Option<&'static str> {
    match err {
        ToolError::TokenCode(CodexErr::Sandbox(SandboxErr::Denied { .. })) => Some("denied"),
        ToolError::TokenCode(CodexErr::Sandbox(SandboxErr::Timeout { .. })) => Some("timed_out"),
        ToolError::TokenCode(CodexErr::Sandbox(SandboxErr::Signal(_))) => Some("signal"),
        ToolError::Rejected(_) | ToolError::TokenCode(_) => None,
    }
}
