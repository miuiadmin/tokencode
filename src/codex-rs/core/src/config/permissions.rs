...
/// for TokenCode to function. These should always be added to the
...
// Adding a new `:special_path` must not make older TokenCode versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of TokenCode. Filesystem access will remain restricted. Upgrade TokenCode if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{}` is not recognized by this version of TokenCode and will be ignored. Upgrade TokenCode if this path is required.",
                subpath.display()
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of TokenCode and will be ignored. Upgrade TokenCode if this path is required."
            ),
        },
    );
}
