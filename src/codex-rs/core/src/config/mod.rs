...
impl Permissions {
    ...
    /// selection. Internal TokenCode-only writable roots are intentionally excluded.
...
// when TokenCode is still responsible for the network policy. `Disabled` means no
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, TokenCode will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - TokenCode
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.tokencode/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "TokenCode"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send TokenCode '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the TokenCode home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that TokenCode can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by TokenCode unless the user explicitly grants access via OS-level keyring access.
    /// file: CODEX_HOME/.credentials.json
    ///       This file will be readable to TokenCode and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, TokenCode will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all TokenCode state (defaults to `~/.tokencode` but can be
    /// overridden by the `CODEX_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Directory where TokenCode stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where TokenCode writes log files (defaults to `$CODEX_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where TokenCode writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores TokenCode version drift between the
    ...
    /// Settings that govern if and what will be written to `~/.tokencode/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current TokenCode executable. This cannot be set in the config
    ...
    /// Whether TokenCode-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for TokenCode updates on startup and surfaces update prompts.
    /// Set to `false` only if your TokenCode updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across TokenCode product surfaces in this machine.
...
impl Config {
    ...
    /// Load a default configuration for a specific TokenCode home without reading
    ...
    /// ignores user settings. For example, the `tokencode exec` subcommand is
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .tokencode/ folders.
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
            ...
            && constrained_approval_policy.value() == AskForApproval::Never
        {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; TokenCode would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode.",
            ));
        }
        if permission_profile_was_constrained {
...
/// Returns the path to the TokenCode configuration directory.
///
/// Resolution order:
/// - `TOKENCODE_HOME` environment variable (primary).
/// - `CODEX_HOME` environment variable (fallback, kept for backward
///   compatibility with existing users and scripts).
/// - If neither is set, defaults to `~/.tokencode`.
///
/// - When a home environment variable is set, the value must exist and be a
///   directory. The value will be canonicalized and this function will Err
///   otherwise.
/// - If no home environment variable is set, this function does not verify
///   that the directory exists.
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    codex_utils_home_dir::find_codex_home()
}

/// Returns the path to the folder where TokenCode logs are stored. Does not verify
/// that the directory exists.
