...
/// Start an interactive sub-TokenCode thread and return IO channels.
///
...
pub(crate) async fn run_codex_thread_interactive(
    ...
    let CodexSpawnOk { codex, .. } = Box::pin(TokenCode::spawn(CodexSpawnArgs {
        config,
...
async fn shutdown_delegate(codex: &TokenCode) {
