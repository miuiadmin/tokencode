...
/// Caller-owned snapshot of TokenCode metadata sent to ResponsesAPI.
///
/// The full TokenCode turn metadata blob is transported canonically as
...
impl CodexResponsesMetadata {
    ...
    fn turn_metadata_payload(&self) -> CodexTurnMetadataPayload<'_> {
        ...
        CodexTurnMetadataPayload {
            ...
            compaction,
            // responsesapi_client_metadata enriches the TokenCode turn metadata blob, not literal
            // top-level Responses client_metadata. Reserved TokenCode-owned keys are filtered when
