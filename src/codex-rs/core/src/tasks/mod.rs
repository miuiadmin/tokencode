...
///
/// Implementations encapsulate a specific TokenCode workflow (regular chat,
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; TokenCode will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
