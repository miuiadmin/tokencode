...
use codex_default_client::CODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
/// Represents a newly created TokenCode thread (formerly called a conversation), including the first event
...
impl ThreadManager {
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        std::fs::create_dir_all(&codex_home)
            .unwrap_or_else(|err| panic!("temp tokencode home dir create failed: {err}"));
    ...
    /// Construct with a dummy AuthManager containing the provided CodexAuth and tokencode home.
    /// Used for integration tests: should not be used by ordinary business logic.
...
impl ThreadManagerState {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        let CodexSpawnOk {
            codex, thread_id, ..
        } = Box::pin(TokenCode::spawn(CodexSpawnArgs {
            config,
