...
/// The high-level interface to the TokenCode system.
...
/// Wrapper returned by [`TokenCode::spawn`] containing the spawned [`TokenCode`] and
...
impl TokenCode {
    /// Spawn a new [`TokenCode`] and initialize the session.
    ...
    async fn spawn_internal(args: CodexSpawnArgs) -> CodexResult<CodexSpawnOk> {
        ...
        let session_configuration = SessionConfiguration {
            ...
            model_provider_id_explicit: config.model_provider_id_explicit,
            collaboration_mode,
        ...
        // Generate a unique ID for the lifetime of this TokenCode session.
    ...
    /// Use sparingly: prefer `submit()` so TokenCode is responsible for generating
...
impl Session {
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect TokenCode performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_environment_cwd) = environment.cwd.to_abs_path() else {
            warn!(
                cwd = %environment.cwd,
                "request_permissions requires a cwd native to the TokenCode host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
