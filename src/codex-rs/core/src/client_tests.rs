...
impl futures::Stream for NotifyAfterEventStream {
    type Item = std::result::Result<UnifiedEvent, UnifiedError>;
    ...
    fn poll_next(mut self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        Poll::Ready(Some(Ok(UnifiedEvent::from(event))))
    }
}
...
async fn dropped_response_stream_traces_cancelled_partial_output() -> anyhow::Result<()> {
    ...
    let api_stream = futures::stream::iter([Ok::<ResponseEvent, ApiError>(
        ResponseEvent::OutputItemDone(item),
    )])
    .chain(futures::stream::pending())
    // 模拟 adapter 输出：在测试边界把 ResponseEvent 归一为中立事件
    //（map_response_events 现消费 UnifiedEvent / UnifiedError）。
    .map(|r| r.map(UnifiedEvent::from).map_err(UnifiedError::passthrough));
...
async fn response_stream_records_last_model_feedback_ids() {
    ...
    let api_stream = futures::stream::iter([
        Ok::<ResponseEvent, ApiError>(ResponseEvent::Created),
...
async fn bedrock_unauthorized_error_uses_provider_mapping() {
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "Amazon Bedrock rejected the request because its AWS signature has expired. Refresh your AWS credentials and retry. If `AWS_BEARER_TOKEN_BEDROCK` is set, update or unset it, then restart TokenCode, url: {url}"
        )
    );
}
