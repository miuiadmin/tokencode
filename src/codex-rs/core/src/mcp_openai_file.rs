//! Bridges Apps SDK-style `openai/fileParams` metadata into TokenCode's MCP flow.
//!
//! Strategy:
...
async fn build_uploaded_argument_value(
    ...
    let Some(auth) = auth else {
        return Err("ChatGPT auth is required to upload files for TokenCode Apps tools".to_string());
    };
