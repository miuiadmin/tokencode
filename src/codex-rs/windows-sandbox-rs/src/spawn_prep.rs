...
mod tests {
    ...
    fn legacy_deny_path_includes_nested_active_root_sid() {
        ...
        let codex_home = temp.path().join("codex-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".tokencode");
    ...
    fn legacy_capability_roots_use_effective_write_roots() {
        ...
        let writable_roots = vec![
            ...
            AbsolutePathBuf::try_from(codex_home.as_path()).expect("tokencode home"),
        ...
        assert!(!roots.contains(&dunce::canonicalize(&codex_home).expect("tokencode home")));
