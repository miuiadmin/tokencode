//! Registry of model providers supported by TokenCode.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so TokenCode works out-of-the-box.
//!   2. User-defined entries inside `~/.tokencode/config.toml` under the `model_providers`
//!      key. These override or extend the defaults at runtime.

use codex_api::Provider as ApiProvider;
use codex_api::RetryConfig as ApiRetryConfig;
use codex_api::is_azure_responses_provider;
pub use codex_language_model::AdapterType;
...
pub enum WireApi {
    ...
    #[default]
    Responses,
    /// OpenAI Chat Completions API at `/v1/chat/completions`（原生多协议支持）。
    /// 非 Responses 后端（含 OpenAI 兼容网关）通过此值路由到 `OpenaiChat` adapter。
    Chat,
    /// Anthropic Messages API at `/v1/messages`（原生多协议支持）。
...
pub struct ModelProviderInfo {
    ...
    /// 模型后端说话方式（驱动 LanguageModel 实现选择）。None 时由 wire_api 推导，
    /// 保持老配置向后兼容。P0 仅 OpenaiResponses 生效，其余变体为 P1+ 预留。
    #[serde(default)]
...
impl ModelProviderInfo {
    ...
    /// 该 provider 应使用的 adapter 类型。显式配置优先；未配置时由 wire_api 推导
    /// （Responses → OpenaiResponses），保证老配置向后兼容。
    pub fn adapter_type(&self) -> AdapterType {
        self.adapter_type.unwrap_or_else(|| match self.wire_api {
...
pub fn built_in_model_providers(
    ...
    // providers are bundled with TokenCode CLI, so we only include the OpenAI and
