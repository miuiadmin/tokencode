...
`codex-app-server-daemon` backs the machine-readable `tokencode app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for TokenCode instances launched over SSH, including fresh developer
...
```sh
tokencode app-server daemon start
tokencode app-server daemon restart
tokencode app-server daemon enable-remote-control
tokencode app-server daemon disable-remote-control
tokencode app-server daemon stop
tokencode app-server daemon version
tokencode app-server daemon bootstrap --remote-control
```
...
```sh
curl -fsSL https://github.com/miuiadmin/tokencode/releases/latest/download/install.sh | sh
$HOME/.tokencode/packages/standalone/current/tokencode app-server daemon bootstrap --remote-control
```
...
The daemon assumes TokenCode is installed through `install.sh` and always launches
the standalone managed binary under `CODEX_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `CODEX_HOME/packages/standalone/current/tokencode` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `CODEX_HOME/packages/standalone/current/tokencode` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `tokencode remote-control` bootstraps with `--remote-control` when the
