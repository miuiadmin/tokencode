...
pub(crate) fn reexec_managed_updater(managed_codex_bin: &std::path::Path) -> Result<()> {
    ...
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed TokenCode binary {}",
...
async fn install_latest_standalone() -> Result<()> {
    let script = reqwest::get("https://raw.githubusercontent.com/miuiadmin/tokencode/main/scripts/install/install.sh")
        .await
        .context("failed to fetch standalone TokenCode updater")?
        .error_for_status()
        .context("standalone TokenCode updater request failed")?
        .bytes()
        .await
        .context("failed to read standalone TokenCode updater")?;
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone TokenCode updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone TokenCode updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone TokenCode updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone TokenCode updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone TokenCode updater exited with status {status}")
    }
}
