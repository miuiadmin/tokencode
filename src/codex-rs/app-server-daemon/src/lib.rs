...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "tokencode app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
impl Daemon {
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by tokencode app-server daemon"
            ));
        }
    ...
    fn ensure_managed_codex_bin(&self) -> Result<()> {
        ...
        Err(anyhow!(
            "managed standalone TokenCode install not found at {managed_codex_path}\n\n\
             This command requires the standalone install managed by the TokenCode installer, because \
             the daemon starts and updates app-server from that fixed path.\n\n\
             Install it with:\n  curl -fsSL https://raw.githubusercontent.com/miuiadmin/tokencode/main/scripts/install/install.sh | sh\n\n\
