...
pub(crate) async fn resolved_managed_codex_bin(codex_bin: &Path) -> Result<PathBuf> {
    fs::canonicalize(codex_bin).await.with_context(|| {
        format!(
            "failed to resolve managed TokenCode binary {}",
            codex_bin.display()
        )
    })
}
...
pub(crate) async fn managed_codex_version(codex_bin: &Path) -> Result<String> {
    let output = Command::new(codex_bin)
        ...
        .with_context(|| {
            format!(
                "failed to invoke managed TokenCode binary {}",
                codex_bin.display()
            )
        })?;
    if !output.status.success() {
        return Err(anyhow!(
            "managed TokenCode binary {} exited with status {}",
            codex_bin.display(),
            output.status
        ));
    }
    ...
    let stdout = String::from_utf8(output.stdout).with_context(|| {
        format!(
            "managed TokenCode version was not utf-8: {}",
            codex_bin.display()
        )
    })?;
...
fn managed_codex_file_name() -> &'static str {
    if cfg!(windows) { "tokencode.exe" } else { "tokencode" }
}
...
fn parse_codex_version(output: &str) -> Result<String> {
    let version = output
        ...
        .ok_or_else(|| anyhow!("managed TokenCode version output was malformed"))?;
