...
async fn exec_appends_piped_stdin_to_prompt_argument() -> anyhow::Result<()> {
    ...
    // echo "my output" | tokencode exec --skip-git-repo-check -C <cwd> -m gpt-5.1 "Summarize this concisely"
...
async fn exec_ignores_empty_piped_stdin_when_prompt_argument_is_present() -> anyhow::Result<()> {
    ...
    // printf "" | tokencode exec --skip-git-repo-check -C <cwd> -m gpt-5.1 "Summarize this concisely"
    test.cmd_with_server(&server)
        .arg("--skip-git-repo-check")
        .arg("-C")
        .arg(test.cwd_path())
...
async fn exec_dash_prompt_reads_stdin_as_the_prompt() -> anyhow::Result<()> {
    ...
    // echo "prompt from stdin" | tokencode exec --skip-git-repo-check -C <cwd> -m gpt-5.1 -
    test.cmd_with_server(&server)
        .arg("--skip-git-repo-check")
        .arg("-C")
        .arg(test.cwd_path())
...
fn exec_without_prompt_argument_rejects_empty_piped_stdin() {
    ...
    // printf "" | tokencode exec --skip-git-repo-check -C <cwd>
    test.cmd()
        .arg("--skip-git-repo-check")
        .arg("-C")
        .arg(test.cwd_path())
