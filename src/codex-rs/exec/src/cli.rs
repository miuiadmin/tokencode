...
#[command(
    version,
    override_usage = "tokencode exec [OPTIONS] [PROMPT]\n       tokencode exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running TokenCode outside a Git repository.
...
struct ResumeArgsRaw {
    ...
    // so "tokencode resume --last <prompt>" treats the positional as a prompt, not a session id.
