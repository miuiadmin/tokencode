...
use codex_default_client::set_default_originator;
...
enum StdinPromptBehavior {
    ...
    /// `tokencode exec` behavior for `tokencode exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `tokencode exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = set_default_originator("codex_exec".to_string()) {
        tracing::warn!(?err, "Failed to set tokencode exec originator override {err:?}");
    }
    ...
    let codex_home = match find_codex_home() {
        ...
        Err(err) => {
            eprintln!("Error finding tokencode home: {err}");
            std::process::exit(1);
        }
    };
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    // Print the effective configuration and initial request so users can see what TokenCode
    // is using.
    ...
    info!("TokenCode initialized with event: {session_configured:?}");
