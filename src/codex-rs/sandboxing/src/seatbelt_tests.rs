...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.tokencode".to_string(),
...
fn create_seatbelt_args_with_read_only_git_and_codex_subpaths() {
    ...
    // write to .tokencode/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.tokencode carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    // Verify that .tokencode/config.toml cannot be modified under the generated
    // Seatbelt policy.
    ...
    // Verify that writing a file to the folder containing .git and .tokencode is allowed.
...
fn create_seatbelt_args_block_first_time_dot_codex_creation_with_metadata_name_regex() {
    ...
    let dot_codex = repo_root.join(".tokencode");
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .tokencode/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".tokencode")).expect("create .tokencode");
    fs::write(
        vulnerable_root.join(".tokencode").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .tokencode/config.toml");
    ...
    let dot_codex_canonical = vulnerable_root_canonical.join(".tokencode");
