...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a TokenCode sandbox."
        );
        return;
    }
...
async fn test_codex_tool_passes_base_instructions() {
    ...
    codex_tool_passes_base_instructions()
        .await
        .expect("tokencode tool should pass base instructions");
}
...
async fn codex_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `tokencode mcp` with a specific config.toml.
        ...
        json!({
            ...
            "structuredContent": {
                "threadId": codex_response
                    ...
                    .expect("tokencode tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow TokenCode to apply proposed code changes?".to_string());
...
/// Create a TokenCode config that uses the mock server as the model provider.
