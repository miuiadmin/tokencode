...
pub struct CodexToolCallParam {
    /// The *initial user prompt* to start the TokenCode conversation.
    pub prompt: String,
...
/// Builds a `Tool` definition (JSON schema etc.) for the TokenCode tool-call.
pub(crate) fn create_tool_for_codex_tool_call_param() -> Tool {
    ...
    let input_schema = create_tool_input_schema(schema, "TokenCode tool schema should serialize");

    Tool::new(
        "codex",
        "Run a TokenCode session. Accepts configuration parameters matching the TokenCode Config struct.",
        input_schema,
    )
    .with_title("TokenCode")
...
impl CodexToolCallParam {
    /// Returns the initial user prompt to start the TokenCode conversation and the
...
pub struct CodexToolCallReplyParam {
    ...
    /// The thread id for this TokenCode session.
    ...
    /// The *next user prompt* to continue the TokenCode conversation.
    pub prompt: String,
}
...
pub(crate) fn create_tool_for_codex_tool_call_reply_param() -> Tool {
    ...
    let input_schema = create_tool_input_schema(schema, "TokenCode reply tool schema should serialize");

    Tool::new(
        "codex-reply",
        "Continue a TokenCode conversation by providing the thread id and prompt.",
        input_schema,
    )
    .with_title("TokenCode Reply")
    .with_raw_output_schema(codex_tool_output_schema())
}
...
mod tests {
    ...
    fn verify_codex_tool_json_schema() {
        ...
        let expected_tool_json = serde_json::json!({
          "description": "Run a TokenCode session. Accepts configuration parameters matching the TokenCode Config struct.",
          "inputSchema": {
            ...
            "properties": {
              ...
              "prompt": {
                "description": "The *initial user prompt* to start the TokenCode conversation.",
                "type": "string"
              },
              "sandbox": {
    ...
    fn verify_codex_tool_reply_json_schema() {
        ...
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Continue a TokenCode conversation by providing the thread id and prompt.",
          "inputSchema": {
            "properties": {
              "conversationId": {
              ...
              "prompt": {
                "description": "The *next user prompt* to continue the TokenCode conversation.",
                "type": "string"
              },
              "threadId": {
                "description": "The thread id for this TokenCode session. This field is required, but we keep it optional here for backward compatibility for clients that still use conversationId.",
                "type": "string"
              }
            },
            "required": [
