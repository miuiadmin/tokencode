//! Asynchronous worker that executes a **TokenCode** tool-call inside a spawned
...
/// To adhere to MCP `tools/call` response format, include the TokenCode
...
/// Run a complete TokenCode session and stream events back to the client.
///
...
pub async fn run_codex_tool_session(
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![Content::text(format!(
                "Failed to start TokenCode session: {e}"
            ))]);
    ...
    // Use the original MCP request ID as the `sub_id` for the TokenCode submission so that
    ...
    loop {
        match thread.next_event().await {
            ...
            Err(e) => {
                let result = create_call_tool_result_with_thread_id(
                    thread_id,
                    format!("TokenCode runtime error: {e}"),
                    Some(true),
                );
                outgoing.send_response(request_id.clone(), result).await;
                break;
