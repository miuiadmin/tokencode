---
name: plugin-creator
description: Create and scaffold plugin directories for TokenCode with a required `.codex-plugin/plugin.json`, optional plugin folders/files, valid manifest defaults, and personal-marketplace entries by default. Use when TokenCode needs to create a new personal plugin, add optional plugin structure, generate or update marketplace entries for plugin ordering and availability metadata, or update an existing local plugin during development with the CLI-driven cachebuster and reinstall flow.
---
...
3. Generate or update the personal marketplace entry when the plugin should appear in TokenCode UI ordering:
...
- If the user specifies a different marketplace path, treat that marketplace as needing explicit installation via `tokencode plugin marketplace add`.
...
- Treat plugin order in `plugins[]` as render order in TokenCode. Append new entries unless a user explicitly asks to reorder the list.
...
- If TokenCode would need approval to write the marketplace file, ask for that approval before
...
- If the user provided a non-default `--marketplace-path`, make sure that marketplace is installed
  before giving reinstall instructions. Use `tokencode plugin marketplace add <path-to-marketplace-root>`
...
- When the workflow created or updated a marketplace-backed plugin, end the final user-facing
  response with a short TokenCode app handoff. Say `To view this in the TokenCode app:` and write
...
- Do not add `pluginName` or `hostId` query parameters to these deeplinks. TokenCode derives both after
