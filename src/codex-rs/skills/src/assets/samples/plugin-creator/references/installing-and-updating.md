...
- the user wants TokenCode to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single TokenCode cachebuster suffix:
...
outside TokenCode depends on a specific token:
...
```bash
tokencode plugin add <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need `tokencode plugin marketplace add` for that
path, and `tokencode plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
```bash
tokencode plugin add <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that TokenCode picks up new skills
   and tools.
...
```text
<base-version>+tokencode.<cachebuster>
```

Examples:

- `0.1.0` → `0.1.0+tokencode.local-20260519-184516`
- `0.1.0+tokencode.old-token` → `0.1.0+tokencode.local-20260519-184516`
- `1.2.3-beta.1+tokencode.prev` → `1.2.3-beta.1+tokencode.local-20260519-184516`
- `dev-build+other-tag` → `dev-build+tokencode.local-20260519-184516`

Replace the existing TokenCode cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `tokencode plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `tokencode plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by TokenCode.
...
- Use `tokencode plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `tokencode plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `tokencode plugin add <plugin-name>@<marketplace-name>`.
