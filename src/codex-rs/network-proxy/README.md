...
`codex-network-proxy` is TokenCode's local network policy enforcement proxy. It runs:
...
`codex-network-proxy` reads from TokenCode's merged `config.toml` (via `codex-core` config loading).
...
When `codex-network-proxy` is embedded in managed TokenCode runtime, policy decisions emit structured
