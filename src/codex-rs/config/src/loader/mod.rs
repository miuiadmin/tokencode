...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/tokencode/config.toml";
...
///
/// - system    `/etc/tokencode/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\TokenCode\requirements.toml` (Windows)
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/tokencode/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\TokenCode\config.toml` (Windows)
...
/// - tree      parent directories up to root looking for `./.tokencode/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.tokencode/config.toml` (loaded but disabled when untrusted)
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/tokencode/requirements.toml"))
}
...
fn windows_codex_system_dir() -> PathBuf {
    ...
    program_data.join("OpenAI").join("TokenCode")
}
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for TokenCode to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".tokencode"))
    }
}
...
async fn load_project_layers(
    ...
    for dir in dirs {
        let dot_codex_abs = dir.join(".tokencode");
...
/// from a file like `/etc/tokencode/managed_config.toml` that has the same
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("TokenCode").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("TokenCode").join("config.toml"))
        );
    }
}
