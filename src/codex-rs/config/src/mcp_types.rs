...
/// OAuth client settings used when TokenCode launches an MCP OAuth flow.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
...
/// Authentication flow TokenCode attempts after resolving an HTTP MCP server's
...
pub struct McpServerConfig {
    ...
    /// Effective environment id for where TokenCode should start this MCP server.
    pub environment_id: String,

    /// When `false`, TokenCode skips initializing this MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,

    /// When `true`, `tokencode exec` exits with an error if this MCP server fails to initialize.
