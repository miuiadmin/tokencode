...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: TokenCode merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
