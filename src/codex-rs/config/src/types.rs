//! Types used to define loaded and effective TokenCode configuration values.
...
/// Determine where TokenCode should store CLI auth credentials.
...
/// Determine where TokenCode should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across TokenCode product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across TokenCode product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in TokenCode rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, TokenCode does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
/// Fallback resize-reflow row cap when TokenCode cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// transcript exceeds this cap. Omit to use TokenCode's terminal-specific default. Set to `0` to
...
/// (primarily the TokenCode IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of TokenCode-managed fast defaults.
...
pub struct PluginMcpServerConfig {
    /// When `false`, TokenCode skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time TokenCode successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision TokenCode last successfully activated for this marketplace.
    #[serde(default)]
