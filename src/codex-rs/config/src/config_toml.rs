//! Schema-heavy configuration TOML types used by TokenCode.
...
/// Base config deserialized from ~/.tokencode/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by TokenCode will likely degrade model performance.
    ...
    /// keyring: Use an OS-specific keyring service.
    /// file: Use a file in the TokenCode home directory.
    ...
    /// Directory where TokenCode writes log files. Setting this value explicitly
    ...
    /// Optional product SKU forwarded on host-owned TokenCode Apps MCP requests.
    ...
    /// directories for `.tokencode` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub struct DebugConfigLockToml {
    ...
    /// Allow replaying a lock generated by a different TokenCode version.
