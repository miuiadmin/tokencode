...
fn model_provider_to_proto(
    ...
    let ModelProviderInfo {
        ...
        supports_websockets,
        adapter_type: _,
        // proto 不承载 max_output_tokens，转换时丢弃（与 adapter_type 同理）。
        max_output_tokens: _,
    } = provider;
...
fn proto_wire_api(wire_api: WireApi) -> proto::WireApi {
    match wire_api {
        ...
        // Anthropic 走本地配置：proto 暂不新增对应变体（远程线程配置无 Anthropic 场景）。
        // 本辅助函数仅用于把本地配置序列化为 proto 供远程同步，Anthropic provider 不会走到这里；
        // 若将来确有需求，应在 proto 补 ANTHROPIC 变体后再放宽此处。
...
mod tests {
    ...
    fn model_provider_proto_roundtrips_chat_wire_api() {
        // 远程线程同步 Chat provider：proto WireApi::Chat ↔ 本地 WireApi::Chat 双向无损。
