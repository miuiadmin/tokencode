...
pub struct CodexPackageLayout {
    ...
    /// Directory containing the TokenCode entrypoint executable.
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.tokencode/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
    ...
    /// A TokenCode binary launched through the npm-managed `codex.js` shim.
    Npm,
    /// A TokenCode binary launched through the bun-managed `codex.js` shim.
    Bun,
    /// A TokenCode binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled TokenCode binaries, custom
    /// internal launchers, and tests that execute TokenCode from an arbitrary path.
    Other,
}
