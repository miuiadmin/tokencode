//! Shared formatting for user-facing `tokencode resume` command hints.
...
pub fn resume_command(thread_name: Option<&str>, thread_id: Option<ThreadId>) -> Option<String> {
    ...
    resume_target.map(|target| {
        ...
        if needs_double_dash {
            format!("tokencode resume -- {escaped}")
        } else {
            format!("tokencode resume {escaped}")
        }
    })
...
pub fn resume_hint(thread_name: Option<&str>, thread_id: Option<ThreadId>) -> Option<String> {
    ...
    match thread_name.filter(|name| !name.is_empty()) {
        Some(thread_name) => Some(format!(
            "tokencode resume, then select {thread_name} ({thread_id})"
        )),
...
mod tests {
    ...
    fn prefers_name_over_id() {
        ...
        assert_eq!(command, Some("tokencode resume my-thread".to_string()));
    }
    ...
    fn formats_thread_id_when_name_is_missing() {
        ...
        assert_eq!(
            command,
            Some("tokencode resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
    ...
    fn quotes_thread_names_when_needed() {
        ...
        assert_eq!(
            command,
            Some("tokencode resume -- -starts-with-dash".to_string())
        );

        let command = resume_command(Some("two words"), /*thread_id*/ None);
        assert_eq!(command, Some("tokencode resume 'two words'".to_string()));

        let command = resume_command(Some("quote'case"), /*thread_id*/ None);
        assert_eq!(command, Some("tokencode resume \"quote'case\"".to_string()));
    }
    ...
    fn resume_hint_names_picker_item_with_id() {
        ...
        assert_eq!(
            hint,
            Some(
                "tokencode resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)"
                    .to_string()
            )
        );
    }
