...
mod tests {
    ...
    fn render_replaces_placeholders_with_and_without_whitespace() {
        let rendered = render(
            ...
            [("name", "TokenCode"), ("place", "codex-rs")],
        )
        .unwrap();

        assert_eq!(
            rendered,
            "Hello, TokenCode. You are in codex-rs. TokenCode is repeated."
        );
    }
    ...
    fn parsed_templates_can_be_reused() {
        ...
        assert_eq!(
            template.render([("greeting", "Hello"), ("name", "TokenCode")]),
            Ok("Hello, TokenCode!".to_string())
        );
        assert_eq!(
    ...
    fn render_supports_literal_delimiter_escapes() {
        ...
        assert_eq!(
            rendered,
            "literal open: {{, literal close: }}, value: TokenCode"
        );
    }
    ...
    fn render_errors_when_extra_value_is_provided() {
        ...
        assert_eq!(
            template.render([("name", "TokenCode"), ("unused", "extra")]),
    ...
    fn render_errors_when_duplicate_value_is_provided() {
        ...
        assert_eq!(
            template.render([("name", "TokenCode"), ("name", "other")]),
    ...
    fn render_function_wraps_parse_errors() {
        let err = render("Hello, }} world.", [("name", "TokenCode")]).unwrap_err();
    ...
    fn render_function_wraps_render_errors() {
        let err = render("Hello, {{ name }}.", [("extra", "TokenCode")]).unwrap_err();
