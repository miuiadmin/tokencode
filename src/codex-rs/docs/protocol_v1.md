...
These are entities exit on the TokenCode backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `TokenCode` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `TokenCode`
   - The core engine of TokenCode
...
2. `Session`
   - The `TokenCode`'s current configuration and state
   - `TokenCode` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `TokenCode` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `TokenCode` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `TokenCode`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `TokenCode`, as `TokenCode` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `TokenCode` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `TokenCode`)
...
- `Event`
  - These are messages sent on the `EQ` (`TokenCode` -> UI)
...
sequenceDiagram
    ...
    box Daemon
    participant codex as TokenCode
