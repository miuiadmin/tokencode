# TokenCode MCP Server Interface [experimental]

This document describes TokenCode's experimental MCP server interface: a JSON-RPC API that runs over the Model Context Protocol (MCP) transport to control a local TokenCode engine.

- Status: experimental and subject to change without notice
- Server binary: `tokencode mcp-server` (or `codex-mcp-server`)
- Transport: standard MCP over stdio (JSON-RPC 2.0, line-delimited)

## Overview

TokenCode exposes MCP-compatible methods to manage threads, turns, accounts, config, and approvals. The types live in `app-server-protocol/src/protocol/{common,v1,v2}.rs` and are consumed by the app server implementation in `app-server/`.
...
Run TokenCode as an MCP server and connect an MCP client:

```bash
tokencode mcp-server | your_mcp_client
```

For a simple inspection UI, you can also try:

```bash
npx @modelcontextprotocol/inspector tokencode mcp-server
```

Use the separate `tokencode mcp` subcommand to manage configured MCP server launchers in `config.toml`.
...
Fetch the catalog of models available in the current TokenCode build with `model/list`. The request accepts optional pagination inputs:
...
- `codex/event` with the serialized TokenCode event payload. The shape matches `core/src/protocol.rs`'s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
The `codex` and `codex-reply` tools return standard MCP `CallToolResult` payloads. For compatibility with MCP clients that prefer `structuredContent`, TokenCode mirrors the content blocks inside `structuredContent` alongside the `threadId`.

Example:

```json
{
  "content": [{ "type": "text", "text": "Hello from TokenCode" }],
  "structuredContent": {
    "threadId": "019bbed6-1e9e-7f31-984c-a05b65045719",
    "content": "Hello from TokenCode"
  }
}
```
...
When TokenCode needs approval to apply changes or run commands, the server issues JSON-RPC requests to the client:
