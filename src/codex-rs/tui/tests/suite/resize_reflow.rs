...
#[ignore = "requires tmux and a locally built tokencode binary; run with --ignored for manual resize smoke"]
...
fn codex_binary(repo_root: &Path) -> Result<PathBuf> {
    ...
    anyhow::ensure!(
        fallback.is_file(),
        "tokencode binary is unavailable; run `cargo build -p codex-cli` first"
    );
    Ok(fallback)
}
