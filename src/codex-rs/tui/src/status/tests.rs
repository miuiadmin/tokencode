...
#[tokio::test]
async fn status_model_provider_uses_bedrock_runtime_base_url_and_hides_usage_link() {
    let temp_home = TempDir::new().expect("temp home");
    let mut config = test_config(&temp_home).await;
    ...
    assert!(
        !rendered.contains("https://chatgpt.com/codex/settings/usage"),
        "expected /status to never show ChatGPT usage link, got: {rendered}"
    );
    ...
    let destinations: Vec<String> = composite
        ...
        .collect();
    assert!(
        !destinations.contains(&"https://chatgpt.com/codex/settings/usage".to_string()),
        "expected no ChatGPT usage hyperlink, got: {destinations:?}"
    );
}
