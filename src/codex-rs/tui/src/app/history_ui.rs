...
const DESKTOP_THREAD_OPENED_MESSAGE: &str = "Opened this session in TokenCode Desktop.";
...
fn desktop_thread_open_error_message(err: &str) -> String {
    format!(
        "Failed to open this session in TokenCode Desktop: {err}. Install or launch TokenCode Desktop and try again."
    )
}
...
fn open_desktop_thread_url(url: &str) -> Result<(), String> {
    ...
    let output = std::process::Command::new("powershell.exe")
        ...
        .arg(&script)
        .output()
        .map_err(|err| format!("failed to launch TokenCode Desktop through PowerShell: {err}"))?;
    ...
    if stderr.is_empty() {
        Err(format!(
            "failed to launch TokenCode Desktop through PowerShell with {}",
            output.status
        ))
    } else {
        Err(stderr)
...
$installLocation = (Get-AppxPackage -Name OpenAI.TokenCode -ErrorAction SilentlyContinue).InstallLocation
if ([string]::IsNullOrWhiteSpace($installLocation)) {{
    Write-Error 'TokenCode Desktop package is not installed'
    exit 1
}}

$appDir = Join-Path $installLocation 'app'
$exe = Join-Path $appDir 'TokenCode.exe'
$app = Join-Path $appDir 'resources\app.asar'
if (-not (Test-Path $exe)) {{
    Write-Error "TokenCode Desktop executable not found at $exe"
    exit 1
}}
if (-not (Test-Path $app)) {{
    Write-Error "TokenCode Desktop app bundle not found at $app"
    exit 1
}}
...
fn open_desktop_thread_url(_url: &str) -> Result<(), String> {
    Err("TokenCode Desktop is only available on macOS and Windows".to_string())
}
