...
mod tests {
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            ...
            absolute_path("/etc/tokencode/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.tokencode")
        } else {
            absolute_path("/repo/.tokencode")
        };
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\TokenCode\\requirements.toml")
        } else {
            absolute_path("/etc/tokencode/requirements.toml")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.tokencode\\config.toml")
        } else {
            absolute_path("/home/alice/.tokencode/config.toml")
        };
        let stack = ConfigLayerStack::new(
