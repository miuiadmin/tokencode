...
impl ChatWidget {
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(
        ...
        header.push(*Box::new(
            Paragraph::new(if allow_unelevated {
                vec![
                    line!["Set up the TokenCode agent sandbox to protect your files and control network access."],
                ]
            } else {
                vec![
                    line!["Your organization requires the default TokenCode agent sandbox to continue. Set it up to protect your files and control network access."],
                ]
            })
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        if allow_unelevated {
            lines.push(line![
                "You can still use TokenCode in a non-admin sandbox. It carries greater risk if prompt injected."
            ]);
        } else {
            lines.push(line![
                "Your organization requires the default sandbox before TokenCode can continue."
            ]);
        }
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                name: "Use TokenCode with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
