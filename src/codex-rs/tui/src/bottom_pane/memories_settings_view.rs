...
impl MemoriesSettingsView {
    pub(crate) fn new(
        ...
        let mut view = Self {
            ...
            complete: false,
            app_event_tx,
            docs_link: Line::from(vec!["Learn more about memories.".dim()]),
            keymap,
        };
    ...
    fn settings_header(&self) -> ColumnRenderable<'_> {
        ...
        header.push(Line::from(
            "Choose how TokenCode uses and creates memories. Changes are saved to config.toml".dim(),
        ));
        header
    }
    ...
    fn reset_confirmation_header(&self) -> ColumnRenderable<'_> {
        ...
        header.push(Line::from(
            "This clears local memory files and rollout summaries for the current TokenCode home."
                .dim(),
        ));
        header
    }
