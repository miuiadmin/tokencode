        ...
        Approved => match subject {
            ...
            ApprovalDecisionSubject::NetworkAccess { target } => (
                ...
                vec![
                    actor.subject().into(),
                    "approved".bold(),
                    " tokencode network access to ".into(),
        ...
        ApprovedExecpolicyAmendment {
            ...
            (
                ...
                vec![
                    actor.subject().into(),
                    "approved".bold(),
                    " tokencode to always run commands that start with ".into(),
                    snippet,
                ],
            )
        }
        ...
        TimedOut => match subject {
            ApprovalDecisionSubject::Command(command) => {
                let summary = if let Some(snippet) = non_empty_exec_snippet(&command) {
                    vec![
                        "Review ".into(),
                        "timed out".bold(),
                        " before tokencode could run ".into(),
                        Span::from(snippet).dim(),
                    ]
                } else {
                    vec![
            ...
            ApprovalDecisionSubject::NetworkAccess { target } => (
                ...
                vec![
                    "Review ".into(),
                    "timed out".bold(),
                    " before tokencode could access ".into(),
                    Span::from(target).dim(),
                ],
            ),
        },
        Abort => match subject {
            ...
            ApprovalDecisionSubject::NetworkAccess { target } => (
                ...
                vec![
                    actor.subject().into(),
                    "canceled".bold(),
                    " the request for tokencode network access to ".into(),
                    Span::from(target).dim(),
                ],
            ),
        },
...
pub fn new_guardian_timed_out_patch_request(files: Vec<String>) -> Box<dyn HistoryCell> {
    let mut summary = vec![
        "Review ".into(),
        "timed out".bold(),
        " before tokencode could apply ".into(),
    ];
    if files.len() == 1 {
        summary.push("a patch touching ".into());
        summary.push(Span::from(files[0].clone()).dim());
