    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for TokenCode".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " - choose what TokenCode is allowed to do".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ TokenCode (vX)"
        let title_spans: Vec<Span<'static>> = vec![
            Span::from(">_ ").dim(),
            Span::from("TokenCode").bold(),
            Span::from(" ").dim(),
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        let mut lines = vec![
            Line::from(format!("TokenCode (v{})", self.version)),
            Line::from(format!(
                "model: {}{}",
                self.model,
