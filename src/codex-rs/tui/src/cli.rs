...
pub struct Cli {
    ...
    // Internal controls set by the top-level `tokencode resume` subcommand.
    ...
    /// top-level `tokencode resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `tokencode fork` subcommand.
    // These are not exposed as user flags on the base `codex` command.
    #[clap(skip)]
    ...
    /// top-level `tokencode fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
