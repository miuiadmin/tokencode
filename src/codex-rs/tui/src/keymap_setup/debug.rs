...
const SHORT_MISSING_KEY_HINT: &str = "Tip: TokenCode can only inspect keys your terminal sends.";
const DELAYED_MISSING_KEY_HINT: &str = "Still waiting? If nothing changes when you press a key, your terminal is not sending that key to TokenCode. Only received keys can be assigned as shortcuts.";
...
impl KeymapDebugView {
    ...
    fn lines_at(&self, width: u16, now: Instant) -> Vec<Line<'static>> {
        ...
        let mut lines = vec![
            ...
            Line::from(
                "Press any key to see what TokenCode receives. Esc is inspected; Ctrl+C closes.".dim(),
            ),
        ];
