//! Tracks when TokenCode-owned transcript scrollback must be repaired after terminal resize.
//!
//! Terminal scrollback is not a retained widget tree: once TokenCode writes wrapped lines into the
//! terminal, the terminal owns those rows. Width resize reflow treats the in-memory transcript cells
//! as the source of truth, clears TokenCode-owned history, and re-emits the cells at the current width.
...
/// TokenCode has already rebuilt scrollback. Keeping those widths distinct lets the next draw request a
