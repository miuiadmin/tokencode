...
//!
//! TokenCode uses the terminal scrollback itself for finalized chat history, so inserting a history
...
/// Zellij does not constrain soft-wrapped continuation rows to TokenCode's scroll region, so its raw
...
mod tests {
    ...
    fn vt100_terminal_wrap_policy_does_not_pre_wrap_long_paragraph() {
        ...
        assert!(
            ...
            "expected terminal soft-wrap instead of TokenCode word pre-wrap, rows: {rows:?}"
        );
    }
