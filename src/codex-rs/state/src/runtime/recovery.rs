//! Backup-and-rebuild support for TokenCode runtime SQLite databases.
//!
//! TokenCode keeps several independent runtime SQLite databases under one SQLite
...
/// Move one TokenCode runtime SQLite database out of the way so that database can
    ...
    match tokio::fs::metadata(sqlite_home).await {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            ...
            Err(std::io::Error::other(format!(
                "no TokenCode runtime database files were found to back up for {}",
                db_path.display()
            )))
        }
        Err(err) => Err(err),
...
async fn backup_sqlite_paths(
    ...
    if backups.is_empty() {
        ...
        return Err(std::io::Error::other(
            "no TokenCode runtime database files were found to back up",
        ));
    }
