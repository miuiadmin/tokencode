...
#[command(about = "Tail TokenCode logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to the TokenCode home directory. Defaults to $TOKENCODE_HOME,
    /// then $CODEX_HOME (backward compat), then ~/.tokencode.
    #[arg(long, env = "TOKENCODE_HOME")]
...
fn default_codex_home() -> PathBuf {
    // Resolution order mirrors find_codex_home: TOKENCODE_HOME, then CODEX_HOME
    // (backward compat), then the default ~/.tokencode directory.
    for var in ["TOKENCODE_HOME", "CODEX_HOME"] {
        if let Some(val) = std::env::var(var)
            .ok()
