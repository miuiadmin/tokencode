//! Aggregates MCP server connections for TokenCode.
//!
...
impl McpConnectionManager {
    ...
    pub async fn new(
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            // For built-in TokenCode Apps, `CODEX_CONNECTORS_TOKEN` is a debug
            ...
            // If TokenCode Apps has an env bearer token, that is its auth path. Do
    ...
    /// Force-refresh tokencode apps tools by bypassing the in-process cache.
    ///
...
fn mcp_init_error_display(
        ...
        && http_headers.as_ref().map(HashMap::is_empty).unwrap_or(true)
    {
        ...
        format!(
            "The {server_name} MCP server is not logged in. Run `tokencode mcp login {server_name}`."
        )
