//! Shared raw tool cache for the host-owned TokenCode Apps MCP server.
//!
//! Cache entries are process-local live state scoped by the active TokenCode auth
...
/// The CodexAuth bits that identify a TokenCode Apps catalog.
///
...
/// Builds the CodexAuth-backed TokenCode Apps cache key.
...
/// Process-scoped registry for shared TokenCode Apps raw tool snapshots.
///
/// Two clients share an entry only when they would read the same TokenCode Apps
...
/// Handle to one shared TokenCode Apps tools cache entry.
///
...
/// Everything that decides whether two TokenCode Apps clients can share tools.
///
...
fn write_cached_codex_apps_tools(
    ...
    .context("failed to serialize TokenCode Apps tools cache")?;
...
fn write_cached_codex_apps_server_info(
    ...
    .context("failed to serialize TokenCode Apps server info cache")?;
...
fn write_codex_apps_cache_file(
    ...
    if let Some(parent) = cache_path.parent() {
        std::fs::create_dir_all(parent).with_context(|| {
            format!(
                "failed to create TokenCode Apps {cache_name} cache directory `{}`",
                parent.display()
            )
        })?;
    }
    std::fs::write(cache_path, bytes).with_context(|| {
        format!(
            "failed to write TokenCode Apps {cache_name} cache `{}`",
            cache_path.display()
        )
    })?;
    Ok(())
...
fn persist_codex_apps_cache(
    ...
    if let Err(err) = &tools_result {
        tracing::warn!("failed to write TokenCode Apps tools cache: {err:#}");
    }
    let server_info_result = write_cached_codex_apps_server_info(cache_context, server_info);
    if let Err(err) = &server_info_result {
        tracing::warn!("failed to write TokenCode Apps server info cache: {err:#}");
    }
