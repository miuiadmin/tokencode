...
//! and exposing cached TokenCode Apps tools while a client is still connecting.
...
/// MCP server capability indicating that TokenCode should include [`SandboxState`]
...
/// Presents declared TokenCode Apps file parameters to the model as local-path inputs and adds plugin
...
/// Converts a TokenCode Apps tool by preserving connector fields, removing connector prefixes from
...
async fn start_server_task(
    ...
    let tools = match (codex_apps_tools_cache_context.as_ref(), fetch_ticket) {
        ...
        (None, None) => tools,
        _ => unreachable!("TokenCode Apps fetch ticket requires cache context"),
    };
    if is_codex_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_LIST_DURATION_METRIC,
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("codex-mcp-client", env!("CARGO_PKG_VERSION")).with_title("TokenCode"),
    )
