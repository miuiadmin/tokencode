...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of TokenCode TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (TokenCode TUI):
    /// - Build this binary: `cargo build -p codex-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `tokencode mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in TokenCode TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            ...
            title: Some("TokenCode Memo".to_string()),
            description: Some(
...
/// Scenarios for `image_scenario`, intended to exercise TokenCode TUI handling of MCP image outputs.
///
