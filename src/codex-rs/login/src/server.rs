...
// Keep in sync with the TokenCode CLI Hydra redirect URI allow-list.
                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], success_url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to TokenCode failed.",
                            io::ErrorKind::Other,
...
/// Returns true when the OAuth callback represents a missing TokenCode entitlement.
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_codex_entitlement_error(error_code, error_description) {
        return "TokenCode is not enabled for your workspace. Contact your workspace administrator to request access to TokenCode.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_codex_entitlement_error(code, error_description) {
            (
                "You do not have access to TokenCode".to_string(),
                "This account is not currently authorized to use TokenCode in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to TokenCode.".to_string(),
                "Contact your workspace administrator to get access to TokenCode, then return to TokenCode and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to TokenCode to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        ...
        assert!(body.contains("You do not have access to TokenCode"));
