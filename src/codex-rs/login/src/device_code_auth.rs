    ...
    if !resp.status().is_success() {
        ...
        if status == StatusCode::NOT_FOUND {
            return Err(io::Error::new(
                ...
                "device code login is not enabled for this TokenCode server. Use the browser login or verify the server URL.",
            ));
        }
...
fn print_device_code_prompt(verification_url: &str, code: &str) {
    ...
    println!(
        "\nWelcome to TokenCode [v{ANSI_GRAY}{version}{ANSI_RESET}]\n{ANSI_GRAY}OpenAI's command-line coding agent{ANSI_RESET}\n\
