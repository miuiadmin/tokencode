...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        ...
        || originator_value.starts_with("TokenCode ")
}
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized TokenCode user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base TokenCode user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default TokenCode originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// TokenCode's route-aware system/PAC resolution.
pub fn create_client() -> CodexHttpClient {
    let inner = build_reqwest_client();
    CodexHttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary TokenCode HTTP traffic.
///
/// This starts from the standard TokenCode user agent, default headers, and sandbox-specific proxy
...
///
/// This supported default path preserves reqwest's existing proxy behavior and does not opt into
/// TokenCode's route-aware system/PAC resolution. Auth callers with route settings must use
...
/// Tries to build the default reqwest client used for ordinary TokenCode HTTP traffic.
///
