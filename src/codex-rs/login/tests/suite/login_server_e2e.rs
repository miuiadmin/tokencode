...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to TokenCode"),
        "error body should clearly explain the TokenCode access denial"
    );
    assert!(
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to TokenCode to retry"),
    ...
    assert!(
        !body.contains("get access to TokenCode"),
