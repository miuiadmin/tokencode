...
fn fixture_paths() -> (TempDir, PathBuf, PathBuf) {
    ...
    let codex_home = root.path().join(".tokencode");
...
async fn detect_repo_still_reports_non_plugin_items_when_home_config_is_invalid() {
    ...
    fs::create_dir_all(&codex_home).expect("create tokencode home");
    fs::write(codex_home.join("config.toml"), "this is not valid = [toml")
        .expect("write invalid tokencode config");
    fs::write(
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root
                        ...
                        .join("settings.json")
                        .display(),
                    repo_root.join(".tokencode").join("config.toml").display()
                ),
                cwd: Some(repo_root.clone()),
                details: None,
            },
        ...
        vec![
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".tokencode").join("hooks.json").display()
                ),
                cwd: Some(repo_root.clone()),
                details: Some(MigrationDetails {
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".tokencode").join("agents").display()
                ),
                cwd: Some(repo_root),
                details: Some(MigrationDetails {
...
async fn import_repo_migrates_mcp_hooks_commands_and_subagents() {
    ...
    let config: TomlValue = toml::from_str(
        &fs::read_to_string(repo_root.join(".tokencode").join("config.toml")).expect("read config"),
    )
    .expect("parse config");
...
"#,
    ...
    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".tokencode").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
...
Research with TokenCode carefully."""
"#,
    )
...
async fn import_repo_mcp_preserves_existing_same_named_server() {
    ...
    .expect("write mcp");
    fs::create_dir_all(repo_root.join(".tokencode")).expect("create tokencode dir");
    let existing_config = r#"[mcp_servers.mixedTransport]
url = "https://example.com/mixed-transport"
"#;
    fs::write(
        repo_root.join(".tokencode").join("config.toml"),
        existing_config,
    )
    .expect("write config");
...
"#,
    ...
    assert_eq!(
        ...
        "Use TokenCode and TokenCode utilities."
    );
}
...
async fn import_repo_mcp_uses_home_settings_toggles_when_repo_settings_missing() {
    ...
    let outcome = service_for_paths(external_agent_home, root.path().join(".tokencode"))
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::McpServerConfig,
            description: String::new(),
            cwd: Some(repo_root.clone()),
...
async fn import_repo_mcp_uses_local_settings_toggles_over_project_settings() {
    ...
    service_for_paths(external_agent_home, root.path().join(".tokencode"))
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::McpServerConfig,
            description: String::new(),
            cwd: Some(repo_root.clone()),
...
async fn detect_repo_does_not_skip_plugins_only_configured_in_project_codex() {
    ...
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".tokencode")).expect("create repo tokencode dir");
    fs::create_dir_all(&codex_home).expect("create tokencode home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
...
"#,
    ...
    .expect("write project tokencode config");
