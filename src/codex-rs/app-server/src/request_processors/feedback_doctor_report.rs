...
//! the configured TokenCode executable as a subprocess, accepts only valid JSON from
//! `tokencode doctor --json`, derives a small set of Sentry tags, and otherwise
...
/// Runs `tokencode doctor --json` and returns a best-effort feedback attachment.
///
/// Failure to spawn TokenCode, finish before the timeout, or parse JSON means the
