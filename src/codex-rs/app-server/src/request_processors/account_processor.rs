...
impl AccountRequestProcessor {
    ...
    async fn get_account_rate_limits_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "tokencode account authentication required to read rate limits",
            ));
        };
        ...
        let response = client
            ...
            .await
            .map_err(|err| internal_error(format!("failed to fetch tokencode rate limits: {err}")))?;
        if response.rate_limits.is_empty() {
            return Err(internal_error(
                "failed to fetch tokencode rate limits: no snapshots returned",
            ));
        }
    ...
    async fn get_account_token_usage_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "tokencode account authentication required to read token usage",
            ));
        };
    ...
    async fn get_workspace_messages_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "tokencode account authentication required to read workspace messages",
            ));
        };
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "tokencode account authentication required to notify workspace owner",
            ));
        };
