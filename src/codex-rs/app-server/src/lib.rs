...
const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "TokenCode rebuilt its local database.";
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        emit_state_db_backup_warning(&format!(
            "TokenCode local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged TokenCode local database file {} to {}",
