...
const CODEX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are TokenCode, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    ...
    let sqlite_home = codex_home_path
        .as_path()
        .to_str()
        .expect("test tokencode home should be utf-8");
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "tokencode unarchive {conversation_id}` to unarchive it first"
            )),
