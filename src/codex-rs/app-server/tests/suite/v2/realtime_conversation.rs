...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from TokenCode.";
...
const RESPONSE_HANDOFF_PREFIX: &str =
    "Silent TokenCode context. Do not speak, acknowledge, or summarize this item.";
...
async fn webrtc_v1_client_managed_handoffs_disable_automatic_output() -> Result<()> {
    ...
    assert!(
        ...
        "automatic TokenCode output should not reach realtime in client-managed handoff mode"
    );
...
async fn websocket_v2_background_agent_steering_ack_requests_response_create() -> Result<()> {
    ...
    // Phase 4: release the gated delegated turn. TokenCode should then continue
