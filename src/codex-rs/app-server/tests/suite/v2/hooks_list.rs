...
"#,
    ...
    std::fs::create_dir_all(workspace.path().join(".tokencode"))?;
    std::fs::write(
        workspace.path().join(".tokencode/config.toml"),
        r#"[features]
hooks = true
...
"#,
    ...
    let project_config_path =
        AbsolutePathBuf::try_from(workspace.path().join(".tokencode/config.toml"))?;
    assert_eq!(
        data,
        vec![
            HooksListEntry {
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    write_project_hook_config(&repo_root.join(".tokencode"), "echo root hook")?;
    write_project_hook_config(&worktree_root.join(".tokencode"), "echo worktree hook")?;
    ...
    let repo_config_path =
        AbsolutePathBuf::from_absolute_path(repo_root.join(".tokencode/config.toml"))?;
