...
`tokencode app-server` is the interface TokenCode uses to power rich interfaces such as the [VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `tokencode app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).
...
The unix socket transport is intended for local app-server control-plane clients. `tokencode app-server proxy`
...
Currently, you can dump a TypeScript version of the schema using `tokencode app-server generate-ts`, or a JSON Schema bundle via `tokencode app-server generate-json-schema`. Each output is specific to the version of TokenCode you used to run the command, so the generated artifacts are guaranteed to match that version.

```
tokencode app-server generate-ts --out DIR
tokencode app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and TokenCode:

- **Thread**: A conversation between a user and the TokenCode agent. Each thread contains multiple turns.
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `codexHome` for the server's TokenCode home directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.
...
Applications building on top of `tokencode app-server` should identify themselves via the `clientInfo` parameter.

**Important**: `clientInfo.name` is used to identify the client for the OpenAI Compliance Logs Platform. If
you are developing a new TokenCode integration that is intended for enterprise use, please contact us to get it
...
```json
{
  ...
  "params": {
    "clientInfo": {
      "name": "codex_vscode",
      "title": "TokenCode VS Code Extension",
      "version": "0.1.0"
    }
  }
}
...
- `thread/realtime/start` — start a thread-scoped realtime session (experimental); pass `outputModality: "text"` or `outputModality: "audio"` to choose model output, optionally pass `model` and, for websocket transport only, `version` to override configured realtime selection for this session only, and pass `includeStartupContext: false` to omit TokenCode's generated startup context. By default, automatic TokenCode text follows the protocol's speakable output path. Pass `clientManagedHandoffs: true` to disable automatic TokenCode response delivery so only the client's explicit append calls produce handoffs. Pass `codexResponsesAsItems: true` to send automatic TokenCode responses as realtime conversation items instead, and optionally pass `codexResponseItemPrefix` to prepend experiment instructions to those items. For V1 sessions, pass `codexResponseHandoffPrefix` while item mode is disabled to route automatic TokenCode commentary through `conversation.handoff.append` with that prefix; final answers remain unprefixed. Returns `{}` and streams `thread/realtime/*` notifications. Omit `transport` for the websocket transport, or pass `{ "type": "webrtc", "sdp": "..." }` to create an AVAS/v1 WebRTC session from a browser-generated SDP offer; the remote answer SDP is emitted as `thread/realtime/sdp`. Explicit `version: "v2"` requests are rejected for WebRTC.
...
- `review/start` — kick off TokenCode’s automated reviewer for a thread; responds like `turn/start` and emits `item/started`/`item/completed` notifications with `enteredReviewMode` and `exitedReviewMode` items, plus a final assistant `agentMessage` containing the review.
...
- `process/spawn` — experimental; spawn a standalone process without the TokenCode sandbox on the host where the app server is running; returns after the process starts and emits `process/outputDelta` and `process/exited` notifications.
...
Start a fresh thread when you need a new TokenCode conversation.
...
Experimental: use `memory/reset` to clear local memory artifacts and sqlite-backed memory stage data for the current TokenCode home. This preserves existing thread memory modes; use `thread/memoryMode/set` separately when a thread's future memory eligibility should change.
...
Turns attach user input (text or images) to a thread and trigger TokenCode generation. The `input` field is a list of discriminated unions:
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.tokencode/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
Then send `offer.sdp` to app-server. Core uses `experimental_realtime_ws_backend_prompt` for the backend instructions and the thread conversation id as the default Realtime API session identifier. This `realtimeSessionId` value refers to the upstream Realtime API session, not a TokenCode session/thread-group id. The start response is `{}`; the remote answer SDP arrives later as `thread/realtime/sdp` and should be passed to `setRemoteDescription()`:
...
Omit `prompt` to use TokenCode's default realtime backend prompt. Send `prompt: null` or
...
is rejected.
Pass `includeStartupContext: false` to skip TokenCode's startup context for this
session while still using the selected backend prompt.
Pass `clientManagedHandoffs: true` to suppress automatic TokenCode response handoffs
...
Pass `codexResponsesAsItems: true` to inject automatic TokenCode responses with
...
experiment instructions to each automatic TokenCode response item. Omit
...
TokenCode commentary through `conversation.handoff.append` and prepends the provided
...
Use `review/start` to run TokenCode’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions TokenCode will run).
...
TokenCode streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set the legacy `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). TokenCode will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `processId` is optional for buffered execution. When omitted, TokenCode generates an internal id for lifecycle tracking, but `tty`, `streamStdin`, and `streamStdoutStderr` must stay disabled and follow-up `command/exec/write` / `command/exec/terminate` calls are not available for that command.
...
Use `process/spawn` to start a standalone argv-based process without the TokenCode sandbox on the host where the app server is running. The `process/*` API is experimental and requires `initialize.params.capabilities.experimentalApi: true`. The spawn response means the process has started and the `processHandle` is registered; completion is reported later through `process/exited`.
...
- `thread/realtime/started` — `{ threadId, realtimeSessionId }` once realtime starts for the thread (experimental). `realtimeSessionId` is the upstream Realtime API session identifier, not a TokenCode session/thread-group id.
...
- `contextCompaction` — `{id}` emitted when TokenCode compacts the conversation history. This can happen automatically.
- `compacted` - `{threadId, turnId}` when TokenCode compacts the conversation history. This can happen automatically. **Deprecated:** Use `contextCompaction` instead.
...
Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell TokenCode whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.tokencode/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a TokenCode skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a TokenCode skill",
                "iconSmall": "icon.svg",
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "/Users/alice/.tokencode/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.tokencode/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.tokencode/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.tokencode/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.tokencode/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
...
TokenCode supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`), which also includes the current ChatGPT `planType` when available, and can be inferred from `account/read`.

- **API key (`apiKey`)**: Caller supplies an OpenAI API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **ChatGPT managed (`chatgpt`)** (recommended): TokenCode owns the ChatGPT OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"` for the browser flow or `type: "chatgptDeviceCode"` for device code; TokenCode persists tokens to disk and refreshes them automatically.
- **Personal access token (`personalAccessToken`)**: TokenCode uses a ChatGPT-backed personal access token loaded outside the app-server login RPCs, such as with `tokencode login --with-access-token` or `CODEX_ACCESS_TOKEN`.
...
- `requiresOpenaiAuth` reflects the active provider; when `false`, TokenCode can run without OpenAI credentials.
- Amazon Bedrock reports `credentialSource: "codexManaged"` when it uses a Bedrock API key managed by TokenCode. Otherwise it reports `credentialSource: "awsManaged"` for the external AWS credential path. This identifies the selected credential source; it does not validate that the AWS credential chain can resolve credentials.
...
`tokencode app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
tokencode app-server generate-ts --out DIR --experimental
tokencode app-server generate-json-schema --out DIR --experimental
```
