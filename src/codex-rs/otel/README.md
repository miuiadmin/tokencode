...
`codex-otel` is the OpenTelemetry integration crate for TokenCode. It provides:
...
that member. If propagated trace context already has the named member, TokenCode
...
TokenCode-specific session events. Rich session/business events should go through
...
to Statsig using TokenCode-internal defaults.
