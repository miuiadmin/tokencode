...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    ...
    metrics.counter_with_description(
        "codex.turns",
        "Total number of TokenCode turns.",
        /*inc*/ 1,
    ...
    metrics.gauge_with_description(
        "codex.active",
        "Number of active TokenCode operations.",
        /*value*/ 2,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
    ...
    assert_eq!(counter.description(), "Total number of TokenCode turns.");
    ...
    assert_eq!(gauge.description(), "Number of active TokenCode operations.");
