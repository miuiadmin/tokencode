//! Defines the protocol for a TokenCode session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for TokenCode:";
...
pub struct ConversationStartParams {
    /// Whether TokenCode response handoffs are managed through explicit client append calls.
    pub client_managed_handoffs: bool,
    /// Sends automatic TokenCode responses as realtime conversation items instead of handoff appends.
    pub codex_responses_as_items: bool,
    /// Optional prefix added to automatic TokenCode response items when `codex_responses_as_items` is set.
    pub codex_response_item_prefix: Option<String>,
    /// Optional prefix added to automatic V1 TokenCode commentary sent with
    ...
    /// Whether to append TokenCode's startup context to the realtime backend prompt.
...
pub enum Op {
    ...
    /// Request TokenCode to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down tokencode instance.
    Shutdown,
...
/// running the command proposed by TokenCode.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// TokenCode errors that we expose to clients.
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::TokenCode),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older TokenCode
...
mod tests {
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::TokenCode])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::TokenCode]));
        assert!(
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".tokencode")).expect("create .codex");
        let canonical_cwd = codex_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_codex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".tokencode"))
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_codex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".tokencode"))
            .expect("canonical .codex");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
