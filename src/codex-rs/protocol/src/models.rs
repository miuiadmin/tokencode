...
pub enum SandboxEnforcement {
    /// TokenCode owns sandbox construction for this profile.
    #[default]
    Managed,
...
/// Filesystem permissions for profiles where TokenCode owns sandbox construction.
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "TokenCode could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "TokenCode cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "TokenCode cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
