...
const PROTECTED_METADATA_CODEX_PATH_NAME: &str = ".tokencode";
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer TokenCode degrades to warn-and-ignore
    /// instead of failing to load. TokenCode 0.112.0 rejected unknown values here,
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".tokencode");
...
mod tests {
    ...
    fn writable_roots_proactively_protect_missing_dot_codex() {
        ...
        let expected_dot_codex = expected_root.join(".tokencode");
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry {
                    path: FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".tokencode".into())),
                    },
                    access: FileSystemAccessMode::Read,
                },
            ])
    ...
    fn writable_roots_skip_default_dot_codex_when_explicit_user_rule_exists() {
        ...
        .expect("absolute canonical root");
        let explicit_dot_codex = expected_root.join(".tokencode");
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_codex_config = cwd.path().join(".tokencode").join("config.toml");
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".tokencode/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let codex_dir = real_root.join(".tokencode");
        ...
        let expected_codex = link_root.join(".tokencode");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_codex = expected_root.join(".tokencode");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let dot_codex = root.join(".tokencode");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let codex_dir = real_tmpdir.join(".tokencode");
