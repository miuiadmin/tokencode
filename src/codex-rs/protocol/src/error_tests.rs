...
fn usage_limit_reached_error_formats_free_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Plus to continue using TokenCode (https://chatgpt.com/explore/plus), or try again later."
    );
}
...
fn usage_limit_reached_error_formats_pro_plan_with_reset() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You've hit your usage limit. Try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_includes_hours_and_minutes() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro) or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using TokenCode, start a free trial of <PLAN> today".to_string(),
            ),
            rate_limit_reached_type: None,
        };
        let expected = format!(
            "You've hit your usage limit. To continue using TokenCode, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
