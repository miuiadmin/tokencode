...
- a `codex-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the TokenCode CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `codex-exec` CLI can check if its arg0 is `codex-linux-sandbox` and, if so, execute as if it were `codex-linux-sandbox`
  - this should also be true of the `tokencode` multitool CLI

On Linux, TokenCode prefers the first `bwrap` found on `PATH`
...
with TokenCode.
TokenCode also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. TokenCode surfaces the same startup warning
...
it cannot create the required user namespaces, so TokenCode rejects sandboxed shell
...
- If `bwrap` is missing, TokenCode also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, TokenCode surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; TokenCode rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.tokencode`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `tokencode sandbox`; the host OS selects the sandbox backend.
