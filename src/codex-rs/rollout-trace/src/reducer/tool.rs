...
impl TraceReducer {
    ...
    fn tool_thread_id(
        ...
        let Some(codex_turn_id) = codex_turn_id else {
            bail!("tool call start did not include thread or TokenCode turn context");
        };
        self.rollout
            .codex_turns
            .get(codex_turn_id)
            ...
            .with_context(|| {
                format!("tool call start referenced unknown TokenCode turn {codex_turn_id}")
            })
    }
    ...
    fn validate_tool_turn(&self, thread_id: &str, codex_turn_id: Option<&str>) -> Result<()> {
        ...
        if let Some(codex_turn_id) = codex_turn_id {
            let Some(turn) = self.rollout.codex_turns.get(codex_turn_id) else {
                bail!("tool call start referenced unknown TokenCode turn {codex_turn_id}");
            };
            if turn.thread_id != thread_id {
                bail!(
                    "tool call start used thread {thread_id}, but TokenCode turn {codex_turn_id} \
                     belongs to {}",
                    turn.thread_id
                );
            }
