...
//! - runtime work starts as soon as TokenCode dispatches the tool.
//!
...
impl TraceReducer {
    ...
    fn start_code_cell(&mut self, pending: PendingCodeCellStart) -> Result<()> {
        ...
        let Some(codex_turn_id) = codex_turn_id else {
            bail!(
                "code cell start {} did not include a TokenCode turn id",
    ...
    pub(super) fn code_cell_event_thread_id(
        ...
        self.rollout
            ...
            .with_context(|| {
                format!(
                    "{event_name} {runtime_cell_id} referenced unknown TokenCode turn {codex_turn_id}"
                )
            })
    }
    ...
    fn validate_code_cell_turn(&self, thread_id: &str, codex_turn_id: &str) -> Result<()> {
        ...
        let Some(turn) = self.rollout.codex_turns.get(codex_turn_id) else {
            bail!("code cell start referenced unknown TokenCode turn {codex_turn_id}");
        };
        if turn.thread_id != thread_id {
            bail!(
                "code cell start used thread {thread_id}, but TokenCode turn {codex_turn_id} belongs \
                 to {}",
                turn.thread_id
            );
        }
