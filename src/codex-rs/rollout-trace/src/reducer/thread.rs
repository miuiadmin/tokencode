...
impl TraceReducer {
    ...
    /// Starts a TokenCode turn inside an existing thread.
    pub(super) fn start_codex_turn(
        ...
        if self.rollout.codex_turns.contains_key(&codex_turn_id) {
            bail!("duplicate tokencode turn start for {codex_turn_id}");
        }
    ...
    /// Marks a TokenCode turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_codex_turn(
        ...
        {
            bail!(
                "tokencode turn end for {codex_turn_id} used thread {event_thread_id}, \
        ...
        let Some(turn) = self.rollout.codex_turns.get_mut(&codex_turn_id) else {
            bail!("tokencode turn end referenced unknown turn {codex_turn_id}");
        };
