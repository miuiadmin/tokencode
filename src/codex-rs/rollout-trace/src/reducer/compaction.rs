...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.codex_turns.get(&started.codex_turn_id) else {
            bail!(
                "compaction request {} referenced unknown tokencode turn {}",
                started.compaction_request_id,
        ...
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but tokencode turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.codex_turn_id,
                turn.thread_id
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.codex_turns.get(&codex_turn_id) else {
            bail!(
                "compaction install {compaction_id} referenced unknown tokencode turn {codex_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but tokencode turn {codex_turn_id} belongs to {}",
                turn.thread_id
            );
        }
