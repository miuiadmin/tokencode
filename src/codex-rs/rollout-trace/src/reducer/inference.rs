...
impl TraceReducer {
    ...
    pub(super) fn start_inference_call(
        ...
        let Some(turn) = self.rollout.codex_turns.get(&codex_turn_id) else {
            bail!(
                "inference start {inference_call_id} referenced unknown tokencode turn {codex_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "inference start {inference_call_id} used thread {thread_id}, \
                 but tokencode turn {codex_turn_id} belongs to {}",
                turn.thread_id
            );
        }
    ...
    /// If a call is still `Running`, TokenCode stopped observing that provider stream
