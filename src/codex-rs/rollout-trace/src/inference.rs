...
///
/// A TokenCode turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one TokenCode turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        codex_turn_id: CodexTurnId,
...
impl InferenceTraceAttempt {
    ...
    /// Records a provider stream that TokenCode intentionally stopped consuming.
    ///
...
/// TokenCode received it, so this helper restores that content in the raw payload.
