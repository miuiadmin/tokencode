...
> **Privacy:** Rollout tracing is not telemetry. TokenCode does **not** upload or
...
during a TokenCode session. It records raw runtime evidence into a local bundle on
...
Hot-path TokenCode code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how TokenCode got
there.
...
TokenCode session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `tokencode debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
TokenCode rollout/session being observed. Keeping those separate lets us reason about
