...
pub struct Arg0DispatchPaths {
    /// Stable path to the current TokenCode executable for child re-execs.
    ///
    ...
    /// instead of the real TokenCode CLI.
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that TokenCode will proceed successfully even if
...
/// While we want to deploy the TokenCode CLI as a single executable for simplicity,
...
///
/// 1.  Load `.env` values from `~/.tokencode/.env` before creating any threads.
...
/// Load env vars from ~/.tokencode/.env.
///
...
/// user to install a separate executable, simplifying the deployment of TokenCode
/// CLI.
