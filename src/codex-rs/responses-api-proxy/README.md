...
# Add this to ~/.tokencode/config.toml:
...
A non-privileged user would then run TokenCode as follows, specifying the `model_provider` dynamically:

```shell
PROXY_PORT=$(jq .port /tmp/server-info.json)
PROXY_BASE_URL="http://127.0.0.1:${PROXY_PORT}"
tokencode exec -c "model_providers.openai-proxy={ name = 'OpenAI Proxy', base_url = '${PROXY_BASE_URL}/v1', wire_api='responses' }" \
...
- Authentication is fixed to `Authorization: Bearer <key>` to match the TokenCode CLI expectations.
...
- We leverage [`codex_process_hardening`](https://github.com/miuiadmin/tokencode/blob/main/src/codex-rs/process-hardening/README.md) so `codex-responses-api-proxy` is run with standard process-hardening techniques.
