...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that TokenCode-managed ChatGPT auth tokens have.
    ...
    ChatgptAuthTokens {
        ...
        ///
        /// When `null`, TokenCode attempts to derive the plan type from access-token
...
pub enum ChatgptAuthTokensRefreshReason {
    /// TokenCode attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that TokenCode was previously using.
    ///
