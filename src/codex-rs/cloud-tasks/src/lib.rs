...
async fn resolve_environment_id(ctx: &BackendContext, requested: &str) -> anyhow::Result<String> {
    ...
    match label_matches.as_slice() {
        [] => Err(anyhow!(
            "environment '{trimmed}' not found; run `tokencode cloud` to list available environments"
        )),
        ...
        [first, rest @ ..] => {
            ...
            if rest.iter().all(|row| row.id == *first_id) {
                ...
                Err(anyhow!(
                    "environment label '{trimmed}' is ambiguous; run `tokencode cloud` to pick the desired environment id"
                ))
            }
        }
    }
...
async fn run_list_command(args: crate::cli::ListCommand) -> anyhow::Result<()> {
    ...
    if let Some(cursor) = page.cursor {
        let command = format!("tokencode cloud list --cursor='{cursor}'");
        if colorize {
            println!(
...
/// Entry point for the `tokencode cloud` subcommand.
