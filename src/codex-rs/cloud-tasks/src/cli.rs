...
pub enum Command {
    /// Submit a new TokenCode Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a TokenCode Cloud task.
    Status(StatusCommand),
    /// List TokenCode Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a TokenCode Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a TokenCode Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in TokenCode Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `tokencode cloud` to browse).
    ...
    /// Git branch to run in TokenCode Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// TokenCode Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// TokenCode Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// TokenCode Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
