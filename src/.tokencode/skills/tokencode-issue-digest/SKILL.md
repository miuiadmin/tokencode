---
name: tokencode-issue-digest
description: Run a GitHub issue digest for miuiadmin/tokencode by feature-area labels, all areas, and configurable time windows. Use when asked to summarize recent TokenCode bug reports or enhancement requests, especially for owner-specific labels such as tui, exec, app, or similar areas.
---

# TokenCode Issue Digest

## Objective

Produce a headline-first, insight-oriented digest of `miuiadmin/tokencode` issues for the requested feature-area labels over the previous 24 hours by default. Honor a different duration when the user asks for one, for example "past week" or "48 hours". Default to a summary-only response; include details only when requested.
...
- Optional repo override, default `miuiadmin/tokencode`
- Optional time window, default previous 24 hours; examples: `48h`, `7d`, `1w`, `past week`

## Workflow

1. Run the collector from a current TokenCode repo checkout:

```bash
python3 .tokencode/skills/tokencode-issue-digest/scripts/collect_issue_digest.py --labels tui exec --window-hours 24
```
...
🔥🔥 Terminal launch hangs on startup [1](https://github.com/miuiadmin/tokencode/issues/123)
🔥 Resume switches model providers unexpectedly [2](https://github.com/miuiadmin/tokencode/issues/456)
...
7. Use inline numbered references where a row or bullet points to issues, for example `Compaction bugs [1](https://github.com/miuiadmin/tokencode/issues/123), [2](https://github.com/miuiadmin/tokencode/issues/456)`. Do not add a separate footnotes section.
...
```text
Use $tokencode-issue-digest to run the TokenCode issue digest for labels tui and exec over the previous 24 hours.
```

```text
Use $tokencode-issue-digest to run the TokenCode issue digest for all areas over the past week.
```
...
```bash
python3 .tokencode/skills/tokencode-issue-digest/scripts/collect_issue_digest.py --all-labels --window "past week" --limit-issues 10
```

Run the focused script tests:

```bash
pytest .tokencode/skills/tokencode-issue-digest/scripts/test_collect_issue_digest.py
```
