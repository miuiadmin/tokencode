#!/usr/bin/env python3
"""Collect recent miuiadmin/tokencode issue activity for owner-focused digests."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collect recent GitHub issue activity for a TokenCode owner digest."
    )
    parser.add_argument(
        "--repo", default="miuiadmin/tokencode", help="OWNER/REPO, default miuiadmin/tokencode"
    )
    parser.add_argument(
        "--labels",
        nargs="+",
...
def clean_title_for_description(title):
    ...
    cleaned = re.sub(
        r"^(tokencode(?: desktop| app|\.app| cli)?|desktop|windows tokencode app)\s*[:,-]\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
