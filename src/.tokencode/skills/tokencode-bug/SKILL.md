---
name: tokencode-bug
description: Diagnose GitHub bug reports in miuiadmin/tokencode. Use when given a GitHub issue URL from miuiadmin/tokencode and asked to decide next steps such as verifying against the repo, requesting more info, or explaining why it is not a bug; follow any additional user-provided instructions.
---
...
Diagnose a TokenCode GitHub bug report and decide the next action: verify against sources, request more info, or explain why it is not a bug.

## Workflow

1. Confirm the input

- Require a GitHub issue URL that points to `github.com/miuiadmin/tokencode/issues/…`.
...
- Prefer the GitHub API over HTML pages because the HTML is noisy:
  - Issue: `https://api.github.com/repos/miuiadmin/tokencode/issues/<number>`
  - Comments: `https://api.github.com/repos/miuiadmin/tokencode/issues/<number>/comments`
...
- **Verify with sources** when the report is specific and likely reproducible. Inspect relevant TokenCode files (or mention the files to inspect if access is unavailable).
