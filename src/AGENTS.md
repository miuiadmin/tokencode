...
- Do not add general product or user-facing documentation to the `docs/` folder. The official TokenCode documentation lives elsewhere. The exception is app-server API documentation, which is covered by the app-server guidance below.
...
TokenCode maintains a context (history of messages) that is sent to the model in inference requests.
...
- Prefer the utilities in `core_test_support::responses` when writing end-to-end TokenCode tests.
...
TokenCode supports running connected app-server and exec-server on different operating systems. See the
